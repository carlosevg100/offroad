...
import {buildMaterialTruthSet,type MaterialReleaseEvidence} from "./truth";
...
const stamps=(authorized:boolean):MaterialReleaseEvidence=>{const fingerprint="b".repeat(64);return {crossValidation:{approved:true,fingerprint},claimAudit:{approved:true,fingerprint},technicalReview:{approved:true,fingerprint,reviewedBy:"desk",reviewedAt:"2026-08-26T00:00:00Z"},companyAuthorization:{authorized,fingerprint:authorized?fingerprint:null,scope:authorized?["qualified_introduction"]:[],recipientIds:authorized?["fund-1"]:[]}}};
...
describe("M7 material truth",()=>{
  ...
  it("authorizes only named recipients against one exact fingerprint",()=>{
    const truth=buildMaterialTruthSet({materials,dataRoom:room,modelAvailable:true,release:stamps(true)});
  ...
  it("blocks a material claim without support",()=>{
    ...
    const truth=buildMaterialTruthSet({materials:[broken,...materials.slice(1)],dataRoom:room,modelAvailable:true,release:stamps(true)});
