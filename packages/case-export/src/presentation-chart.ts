...
export function chartWorkbook(series: Series): Uint8Array {
  ...
  const rows = `<row r="1">${str("A1", "Period")}${str("B1", series.unit ?? "Value")}</row>` + series.points.map((point,i) => `<row r="${i+2}">${str(`A${i+2}`,point.label)}${point.value === null ? "" : `<c r="B${i+2}"><v>${point.value}</v></c>`}</row>`).join("");
  return zipStored([
...
export function nativeChartXml(series: Series): string {
  ...
  const ser=`<c:ser><c:idx val="0"/><c:order val="0"/><c:tx><c:v>${x(series.unit ?? "Value")}</c:v></c:tx><c:spPr>${line?'<a:ln w="28575"><a:solidFill><a:srgbClr val="7D9455"/></a:solidFill></a:ln>':'<a:solidFill><a:srgbClr val="7D9455"/></a:solidFill>'}</c:spPr>${!line?'<c:invertIfNegative val="0"/>':""}${!line?series.points.map((p,i)=>p.value!==null&&p.value<0?`<c:dPt><c:idx val="${i}"/><c:invertIfNegative val="0"/><c:spPr><a:solidFill><a:srgbClr val="A23B3B"/></a:solidFill></c:spPr></c:dPt>`:"").join(""):""}${cat}${val}${line?'<c:smooth val="0"/>':""}</c:ser>`;
  return `${head}<c:chartSpace xmlns:c="http://schemas.openxmlformats.org/drawingml/2006/chart" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><c:chart><c:autoTitleDeleted val="1"/><c:plotArea><c:layout/><c:${chartTag}>${line?'<c:grouping val="standard"/>':`<c:barDir val="${series.chartKind==="bar"?"bar":"col"}"/><c:grouping val="clustered"/>`}${ser}<c:axId val="101"/><c:axId val="102"/></c:${chartTag}><c:catAx><c:axId val="101"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:axPos val="${series.chartKind==="bar"?"l":"b"}"/><c:tickLblPos val="low"/><c:crossAx val="102"/><c:crosses val="autoZero"/><c:auto val="1"/><c:lblAlgn val="ctr"/></c:catAx><c:valAx><c:axId val="102"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:axPos val="${series.chartKind==="bar"?"b":"l"}"/><c:majorGridlines/><c:numFmt formatCode="#,##0.##;(#,##0.##);–" sourceLinked="0"/><c:tickLblPos val="nextTo"/><c:crossAx val="101"/><c:crosses val="autoZero"/><c:crossBetween val="between"/></c:valAx></c:plotArea><c:plotVisOnly val="1"/><c:dispBlanksAs val="gap"/></c:chart><c:externalData r:id="rId1"><c:autoUpdate val="0"/></c:externalData></c:chartSpace>`;
}
