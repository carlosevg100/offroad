...
describe("planDataRoom", () => {
  it("places the teaser before the NDA, the term sheet after it, and the memo with the desk", () => {
    const plan = planDataRoom({materials: ["teaser", "credit_profile", "term_sheet", "diligence_qa", "package", "investment_memo"].map((k) => material(k as Material["kind"])), materialsBlockedBy: [], documents: [verified], exceptions: [], readiness: readiness(), disclosureAuthorised: true});
  ...
  it("holds every external entry behind a blocking exception, and never the internal memo", () => {
    const plan = planDataRoom({materials: [material("teaser"), material("investment_memo")], materialsBlockedBy: [], documents: [verified], exceptions: [exception(true)], readiness: readiness(), disclosureAuthorised: true});
  ...
  it("holds unverified and unclassified files, and lists what the case still needs as requests", () => {
    const plan = planDataRoom({materials: [material("teaser")], materialsBlockedBy: ["audit refused the brief"], documents: [unverified, unclassified], exceptions: [exception(false)], readiness: readiness([{id: "debt_schedule", labels: {pt: "Mapa de dívida", en: "Debt schedule"}}]), disclosureAuthorised: false});
    const byId = new Map(plan.entries.map((entry) => [entry.id, entry]));
    ...
    expect(byId.get("material:term_sheet")?.heldBy.map((h) => h.en)).toEqual(["audit refused the brief", "The company has not yet authorised disclosure of its name"]);
  ...
  it("renders an internal index with one table per folder and the hash of each file", () => {
    const plan = planDataRoom({materials: [material("teaser")], materialsBlockedBy: [], documents: [verified], exceptions: [], readiness: readiness(), disclosureAuthorised: true});
