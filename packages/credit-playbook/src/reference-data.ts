 ...
 */
export const referenceDataRegistryVersion = "2026.09.05-v9";
...
const baseReferenceDataRegistry = [
  ...
  draft("policy.reconciliation.tolerance", "methodology_parameter", "Numerical tolerance by statement, currency, scale and period for reconciliation gates.", "Head de Análise Financeira", ["Q-13", "Q-16", "Q-17"], {"value": {"absolute": "0", "families": {"working_capital": "1000", "net_debt": "1000", "interest": "2000"}, "familiesUnit": "BRL thousand", "rule": "toda diferença é nomeada e explicada; a tolerância por família cobre só o arredondamento entre uma fonte em R$ milhões com uma casa (release) e uma em R$ mil (notas), ou a agregação de juros e variações; nenhuma tolerância acima de zero sem chave e versão de política no resultado", "roundingExplanation": "diferença igual ao arredondamento da fonte é explicada como arredondamento, não tolerada"}, "unit": "unidade da demonstração", "source": {"title": "Regra do fundador (4 set 2026): nenhuma diferença fica sem nome; executor build-debt-ledger v4, tolerância só sob política versionada", "observedBy": "agente Offroad, proposta a partir do gabarito 01 v0.8 e dos executores do Caso 01"}, "asOf": "2026-09-05"}),
  ...
  draft("policy.structure.maturity_wall", "methodology_parameter", "Share of gross debt maturing in one period above which the period is a wall, and the rule that the threshold is strict.", "Head de DCM e Estruturação", ["D-03", "D-05", "D-28"], {"value": {"shareOfGrossDebt": "0.20", "rule": "estritamente acima do limiar; igual ao limiar não é parede; denominador é a dívida bruta da nota"}, "unit": "fração da dívida bruta", "source": {"title": "Gabarito 01 v1.0, seção 3 (dois picos de cerca de 1,23 bilhão, 21,7% da dívida bruta); limiar proposto para aprovação", "observedBy": "agente Offroad, proposta a partir do gabarito 01 e do executor diagnose-maturity-wall v2"}, "asOf": "2026-09-05"}),
