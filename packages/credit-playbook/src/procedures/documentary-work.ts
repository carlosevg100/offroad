...
export const documentaryWorkMethod = {id: "documentary-work-pipeline", version: "2026.09.10-v12"} as const;
export const documentaryWorkProcedures = [canonicalProcedureSchema.parse({
  ...
  procedure: [
    ...
    {id: "source_review", title: "Revisar fidelidade das interpretações", mode: "model_assisted", instructions: [
      ...
      "Produce one fieldAssessment for EVERY authoredField in input order, independently in its own language, before compiling issues. Each assessment must quote an exact local excerpt and choose source_supported, no_factual_assertion or unsupported. Source_supported requires sourceIds that actually establish ALL factual premises in that field. No_factual_assertion is only for neutral labels, genuinely open questions or explicitly hypothetical antecedents; it cannot cover a categorical assertion merely because another sentence is conditional. Every unsupported assessment requires an anchored issue for that same field; every issue requires an unsupported assessment. Listing reviewedFieldIds alone is not a review.",
      "Do not stop after the first violation or after reviewing one language. Recheck every title, hypothesis text, hypothesis question, gap text and gap question separately. A correct conditional sentence about insurance in one item cannot establish a covenant's absence in another. 'The agreement has no leverage covenant' is an unsupported factual assertion when the source says only that covenant terms were not supplied; uncertainty about a possible consequence ('may reduce protection') does not make that asserted antecedent conditional. Apply the same distinction in Portuguese and other requested source languages.",
