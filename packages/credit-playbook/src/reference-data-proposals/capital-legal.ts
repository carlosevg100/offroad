...
const accelerationEvents: ReferenceDataProposal = {
  ...
  value: {
    ...
    materialityConventions: {
      ...
      financialDebtDefinition: "empréstimos, financiamentos, títulos de dívida, derivativos pelo valor de liquidação e garantias prestadas a dívida financeira de terceiros; exclui fornecedores no curso normal, parcelamento tributário em dia e contingência em discussão",
      ...
      cureClasses: {source: "policy.structure.cure-waiver", classes: ["payment", "information", "financial_covenant", "other_non_monetary", "collateral_top_up", "license_restoration", "none"], precedence: "prazo definido em policy.structure.cure-waiver para a mesma classe prevalece sobre as janelas deste catálogo"},
    },
    events: [
      {id: "payment_default", mode: "automatic", trigger: "não pagamento de principal, juros ou qualquer valor devido na data", threshold: "qualquer valor", cureClass: "payment", basis: "contrato; Código Civil, art. 397"},
      ...
      {id: "grave_integrity_breach", mode: "automatic", trigger: "condenação definitiva, administrativa ou judicial, da emissora ou de controlada por ato lesivo da Lei 12.846/2013, trabalho infantil ou trabalho análogo ao de escravo", threshold: "qualquer", cureClass: "none", basis: "Lei 12.846/2013; contrato"},
      {id: "non_monetary_breach", mode: "non_automatic", trigger: "descumprimento de obrigação não pecuniária", threshold: "qualquer", cureClass: "other_non_monetary", basis: "contrato"},
      {id: "information_breach", mode: "non_automatic", trigger: "atraso ou falha na entrega de informação obrigatória", threshold: "qualquer", cureClass: "information", basis: "contrato; ES-30"},
      {id: "financial_covenant_breach", mode: "non_automatic", trigger: "índice financeiro fora do limite na data de apuração, pela definição do contrato", threshold: "limite do covenant", cureClass: "financial_covenant", basis: "contrato; ES-23 e ES-24"},
      {id: "cross_default", mode: "non_automatic", trigger: "inadimplemento de outra dívida financeira não sanado no prazo de cura daquele instrumento, sem vencimento antecipado declarado", threshold: "limiar monetário do cross-default, individual ou agregado", cureClass: "none", basis: "contrato; ES-28"},
      {id: "incorrect_representation", mode: "non_automatic", trigger: "declaração incorreta, incompleta ou enganosa em aspecto relevante", threshold: "aspecto relevante", cureClass: "other_non_monetary", basis: "contrato"},
      ...
      {id: "essential_license_loss", mode: "non_automatic", trigger: "perda, suspensão, cassação ou não renovação de licença ou autorização essencial à atividade", threshold: "licença essencial", restorationDays: 30, cureClass: "license_restoration", basis: "contrato; EMP-17"},
      {id: "collateral_loss", mode: "non_automatic", trigger: "desapropriação, perda ou deterioração de ativo relevante ou de ativo em garantia sem reforço no prazo", threshold: "ativo relevante ou garantia", topUpDays: 30, cureClass: "collateral_top_up", basis: "Código Civil, arts. 333, III, e 1.425"},
      {id: "collateral_coverage_shortfall", mode: "non_automatic", trigger: "garantia abaixo da cobertura mínima sem recomposição no prazo da régua", threshold: "policy.structure.collateral-coverage", cureClass: "collateral_top_up", basis: "contrato; ES-11 e ES-20"},
      ...
      {id: "security_not_perfected", mode: "non_automatic", trigger: "garantia não constituída, não registrada ou sem eficácia no prazo contratado", threshold: "qualquer garantia do pacote", cureClass: "collateral_top_up", basis: "Lei 6.404/1976, art. 62, III; contrato"},
      ...
      {id: "other_integrity_breach", mode: "non_automatic", trigger: "descumprimento de legislação ambiental, trabalhista ou anticorrupção com efeito adverso relevante, fora do evento automático", threshold: "efeito adverso relevante", cureClass: "other_non_monetary", basis: "contrato"},
    ],
    declarationProcedure: {
      ...
      commercialNotes: "mudança de condições por maioria simples dos titulares presentes, salvo quórum maior no termo, com as regras de assembleia de debenturistas (Lei 14.195/2021, art. 47, §§ 2º e 3º)",
    },
    ...
    materialAdverseChange: {
      ...
      objectiveCriteria: [
        {id: "ebitda_drop", rule: "EBITDA dos últimos 12 meses abaixo do caso base do fechamento", dropShare: "0.25"},
        {id: "rating_downgrade", rule: "rebaixamento de rating em escala nacional", notches: 2},
        {id: "customer_loss", rule: "perda de cliente ou contrato que responda pela fração da receita dos últimos 12 meses", revenueShare: "0.20"},
    ...
    calibrationRules: [
      ...
      "o mesmo limiar monetário vale para cross-default, protesto e decisão judicial",
...
const corporateAuthority: ReferenceDataProposal = {
  ...
  unit: "matriz de competência por forma societária e ato; limites em fração decimal; prazos em dias",
  source: {
  ...
  value: {
    ...
    groupGuarantees: {
      ...
      newGuaranteeForExistingDebt: {legalTermDays: 90, rule: "garantia real constituída no termo legal para dívida anterior é ineficaz perante a massa", article: "Lei 11.101, arts. 99, II, e 129, III"},
...
const intercreditor: ReferenceDataProposal = {
  ...
  value: {
    ...
    housePosition: {
      ...
      paymentBlockage: {duringSeniorPaymentDefault: true, otherSeniorEventMaxDays: 180, maxOncePerDays: 360},
...
const routeCatalogue: ReferenceDataProposal = {
  ...
  value: {
    ...
    legacyCorrections: [
      "rito automático é oferta pública (Resolução CVM 160, art. 26); o mapeamento legado o chama de colocação privada",
      "o catálogo legado cita os limites de 75 investidores e 50 subscritores da revogada Instrução CVM 476",
      "o catálogo legado trata a FINAME como sujeita a IOF; operação com recursos da FINAME tem alíquota zero (Decreto 6.306, art. 8º, IX)",
    ],
...
const instrumentEligibility: ReferenceDataProposal = {
  ...
  value: {
    ...
    instruments: [
      ...
      {id: "commercial_note", legacyInstrumentId: null, routeIds: ["commercial_note"], issuers: ["sa_aberta", "sa_fechada", "ltda", "cooperativa"], conditions: ["escrituração por instituição autorizada pela CVM", "aprovação pelos órgãos de administração ou pelo administrador"], ceiling: {type: "none"}, termConstraints: ["mudança de condições por maioria simples dos titulares presentes, salvo quórum maior (Lei 14.195, art. 47, § 2º)", "agente fiduciário quando a CVM exigir (art. 50)", "oferta privada pode prever conversão em participação, exceto sociedade anônima (art. 51, § 2º)"], basis: "Lei 14.195/2021, arts. 45 a 51"},
      {id: "cri", legacyInstrumentId: "cri", routeIds: ["cri"], issuers: ["securitizadora"], debtorConditions: {debtSecurityDebtorPrincipalSector: "imobiliário", principalSectorRevenueShare: {numerator: 2, denominator: 3}, comparator: "acima de", excluded: ["instituição financeira, seu conglomerado prudencial e controladas", "operação entre partes relacionadas", "estrutura em que pessoa excluída retém riscos e benefícios"], notDebtSecurities: ["contrato de locação", "compra e venda e promessa de venda de imóvel", "duplicata", "usufruto"]}, conditions: ["lastro identificado e adquirido até a integralização", "regime fiduciário obrigatório na oferta ao público em geral (Resolução CVM 60, Anexo I, art. 4º)"], ceiling: {type: "eligible_backing", basis: "Lei 14.430/2022, art. 22, § 3º"}, grandfathering: "emissões distribuídas ou protocoladas na CVM antes de cada mudança da Resolução CMN 5.118", basis: "Lei 14.430/2022; Resolução CMN 5.118/2024, art. 3º, na redação das Resoluções 5.121/2024, 5.163/2024 e 5.212/2025"},
      {id: "cra_cdca", legacyInstrumentId: "cra", routeIds: ["cra", "cdca"], issuers: ["securitizadora (CRA)", "cooperativa agropecuária ou pessoa jurídica que comercializa, beneficia ou industrializa produto, insumo, máquina ou implemento agropecuário (CDCA)"], debtorConditions: {debtSecurityDebtorPrincipalSector: "agronegócio", principalSectorRevenueShare: {numerator: 2, denominator: 3}, comparator: "acima de", excluded: ["instituição financeira, seu conglomerado prudencial e controladas", "operação entre partes relacionadas"]}, conditions: ["recebíveis do CDCA registrados ou depositados (Lei 11.076, art. 25, § 1º, I)"], ceiling: {type: "eligible_backing"}, basis: "Lei 11.076/2004, arts. 24 e 25; Resolução CMN 5.118/2024 na redação vigente"},
      {id: "fidc", legacyInstrumentId: "fidc", routeIds: ["fidc"], issuers: [...allCompanies, "pessoa_natural"], conditions: ["direitos creditórios registrados em registradora autorizada pelo Banco Central ou custodiados (Anexo II, arts. 30, I, e 37)", "até 20% do patrimônio por devedor ou grupo, ampliável em classe de investidor qualificado nas hipóteses do art. 45, § 3º", "público em geral: cota sênior com rating, cronograma de amortização e nenhuma cota subordinada ao público (art. 13)", "recebível não padronizado só para investidor profissional (art. 15)", "recebível de cedente em recuperação judicial ou extrajudicial segue padronizado (Resolução CVM 240/2026)"], perDebtorLimit: "0.20", minimumReceivablesShareWithin180Days: "0.50", ceiling: {type: "eligible_pool_after_subordination", formula: "cotas_senior_e_mezanino <= carteira_elegivel * (1 - subordinacao_exigida)", haircutKey: "policy.structure.collateral_haircuts"}, basis: "Resolução CVM 175/2022, Anexo Normativo II"},
...
const backgroundSources: ReferenceDataProposal = {
  ...
  value: {
    ...
    prohibited: {
      ...
      sources: ["perfis pessoais em redes sociais", "bases vazadas ou de origem não comprovada", "área logada ou credencial de terceiro"],
