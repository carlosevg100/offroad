...
export type MethodValueType =
  | {type: "string" | "decimal_string" | "boolean" | "integer" | "date"}
...
export const methodValueTypeSchema: z.ZodType<MethodValueType> = z.lazy(() => z.discriminatedUnion("type", [
  z.object({type: z.enum(["string", "decimal_string", "boolean", "integer", "date"])}).strict(),
  ...
  z.object({type: z.literal("object"), fields: z.record(componentIdSchema, z.object({required: z.boolean(), value: methodValueTypeSchema}).strict()).refine((fields) => Object.keys(fields).length > 0, "object fields must be typed")}).strict(),
]));
