...
- **Valor proposto.** Grade versionada da casa, vazia em 24/09/2026: nenhuma célula publicada e nenhuma observação governada incluída nesta proposta. O valor são as regras de construção:
  ...
  - Chave da célula: faixa de risco × família de instrumento × faixa de prazo × família de garantia.
    ...
    - Família de instrumento: bancária (`ccb`, `nce`, `leasing`), mercado de capitais (`debenture_476`, `debenture_160`, `nota_comercial`), securitização (`cri`, `cra`), fundo de recebíveis (`fidc`), `venture_debt` e fomento (`finame`). Fomento tem custo subsidiado e nunca se compara com mercado. `debenture_476` é a chave legada que nomeia a debênture para investidor profissional pelo rito automático da Resolução CVM 160; `nota_comercial` está no vocabulário do motor de preço desde 24/09/2026, e a tabela `pricing_observations` ainda não aceita observação com essa chave.
    - Faixa de prazo, em meses: 1 a 24; 25 a 36; 37 a 60; 61 a 84; 85 a 120; 121 a 360.
  ...
  - Atualização: ingestão das fontes públicas diária, em dia útil; relatório de frescor no primeiro dia útil de cada mês; recálculo a cada nova observação elegível, a cada gatilho de `policy.pricing.regime` e a cada movimento de 30 pontos-base na medida de referência.
...
- **Regra de aplicação.**
  ...
  7. Executor atual: `buildPricingTruthSet` rejeita observação vencida, de outro regime, sem autorização de uso agregado ou abaixo da qualidade, e só aceita CDI como indexador. Desde 24/09/2026 ele exige a mesma faixa de risco, o mesmo instrumento e a mesma classe de garantia, aplica a janela de prazo e a razão de tamanho de `policy.pricing.sample-quality`, pondera setor, amortização e recência sem excluir a observação e forma a banda pelos P25 e P75 ponderados; sem base, abstém-se com o motivo. Não funde células e não calcula a confiança: nesses dois pontos continua mais restritivo que a regra da casa e erra para a abstenção, nunca para a precisão inventada.
...
- **Revisão.** Qualquer gatilho de `policy.pricing.regime`; relatório mensal com mais da metade das células publicadas vencidas; mudança da classificação interna de risco; aceitação de `nota_comercial` na tabela `pricing_observations` ou troca da chave legada `debenture_476`; fusão de células e confiança implementadas no executor; revisão trimestral com os casos de ouro.
...
- **Valor proposto.**
  - Para o executor (`pricing_policies` e `buildPricingTruthSet`): `minObservations` 5, `minDistinctSources` 5, `minQuality` 0,75, `maxTenorDeltaMonths` 12, `tenorWindowFloorMonths` 6, `tenorWindowRelative` 0,5, `minAmountRatio` 0,5 e `maxAmountRatio` 2.
  ...
  - Tabela de vizinhança, valor de cada dimensão:
    ...
    - Amortização: a mesma 1; SAC contra Price 0,9; bullet contra amortizável 0,5. A carência não tem valor próprio na tabela: o peso de 0,05 de amortização e carência do Caso 01 é aplicado sobre a forma de amortização.
...
- **Regra de aplicação.**
  ...
  7. Executor atual, desde 24/09/2026: aplica `minObservations`, `minDistinctSources`, `minQuality` e a razão de tamanho; mede a janela de prazo pela fórmula desta chave sobre o prazo nominal em meses; exige a mesma faixa de risco, o mesmo instrumento e a mesma classe de garantia; calcula o score com os pesos desta chave, o fator de recência por tipo de fonte e o peso de cada observação; recusa score abaixo de 0,80, de modo que só comparáveis diretos entram; e tira os quartis ponderados. Ainda não usa o macrossetor (outro setor recebe 0,3), a fusão de células nem a escala de confiança, o que o deixa mais restritivo que a política.
...
- **Valor proposto.**
  - Para o executor: `minBandWidthBps` 15 e `maxBandWidthBps` 150, o envelope absoluto para CDI, único indexador que o executor aceita.
  ...
  - Texto padrão: "Referência indicativa de {indexador} + {mínimo}% a {máximo}% ao ano, sujeita à análise e à decisão dos investidores. Base: {n} observações de {k} origens entre {data inicial} e {data final}; confiança {nível}." Em CDI, o indexador é DI; em IPCA e prefixado, é o título de referência, com a taxa total equivalente e a data da taxa indicativa (regra 7).
- **Regra de aplicação.**
  ...
  9. Executor atual: desde 24/09/2026 mede a largura entre os P25 e P75 ponderados e a compara com `minBandWidthBps` e `maxBandWidthBps`; abaixo do piso, abstém-se em vez de alargar, e ainda não aplica o piso relativo, o multiplicador de confiança, as etapas de aproximação nem o arredondamento para fora. Os dois comportamentos impedem banda abaixo do piso; a diferença fica para o alinhamento do método.
  - Exemplos:
...
- **Valor proposto.**
  ...
  - Condições de referência registradas na declaração:
    ...
    - Taxas indicativas ANBIMA de 23/09/2026: as cinco LTN e as cinco NTN-B do contexto listado de `market.pricing.indexer-basis`, de LTN 01/04/2027 a 13,3457% a LTN 01/01/2032 a 14,1561%, e de NTN-B 15/08/2028 a 7,3971% a NTN-B 15/05/2035 a 7,6161%.
...
- **Valor proposto.** Curva governada vazia em 24/09/2026, com método, degraus e contexto público definidos:
  ...
  - Faixas de prazo: as da grade (1 a 24; 25 a 36; 37 a 60; 61 a 84; 85 a 120; 121 a 360 meses).
...
- **Valor proposto.**
  ...
  - Observação pública, ofertas encerradas registradas pela CVM de 24/09/2025 a 23/09/2026:
    ...
    - Investidores no encerramento fora do consórcio de distribuição, mediana entre as ofertas que informaram, por faixa de R$ milhões: debêntures de 20 a 50, 3,5 (18 ofertas); de 50 a 150, 7 (P25 3, P75 21, 51 ofertas); de 150 a 500, 15 (100); acima de 500, 63,5 (122). CRI e CRA até 20, 2 (119); de 20 a 50, 2 (129); de 50 a 150, 3 (137). Cotas de FIDC até 20, 2 (395); de 20 a 50, 4 (205); de 50 a 150, 8 (159); de 150 a 500, 16 (77); acima de 500, 60,5 (38).
...
- **Valor proposto.**
  ...
  - Níveis para `pricing_policies.index_levels`: `cdi` 0,1365; `ipca` 0,0422; `tlp` 0,0830; `tr` 0,001668; `source_id` bcb-sgs:4389;13522;27572;226; `observed_on` 2026-09-22; `valid_until` 2026-09-30.
    ...
    - `tr`: TR do período de 22/09/2026 a 22/10/2026, fração ao mês. O Banco Central publica uma TR para cada dia de início de período; como nível do dia, a de 22/09/2026 vale até 23/09/2026, e a TR de outro dia de início é lida no SGS 226. Por isso a validade do conjunto, 30/09/2026, é a menor entre as de `cdi`, `ipca` e `tlp`, e o executor, que só aceita `cdi`, não lê a TR.
...
- **Regra de aplicação.**
  ...
  7. Executor atual: `buildPricingTruthSet` só aceita `cdi` e, desde 24/09/2026, compõe a taxa total pela regra 3 com `composeIndexAndSpread` do financial-core. A soma linear, que o executor usava antes, subestima a taxa em cerca de DI × spread: com DI de 13,65%, 14 pontos-base para spread de 1,00% e 41 para spread de 3,00%.
...
- **Uso no método.**
  ...
  - D-02: normalização da dívida existente pelas mesmas conversões, com o ponto de curva substituto de `policy.capital.anbima-b3-conventions` quando o fluxo do contrato não permite a duration.
...
- **Valor proposto.**
  ...
  - Estado de cada custo: conhecido (contrato, tabela oficial ou cotação), estimado (provisão da casa), zero, não aplicável ou desconhecido. Custo desconhecido bloqueia o all-in da alternativa e nunca recebe zero nem estimativa do modelo; custo estimado sustenta só a leitura preliminar. Estes estados e a hierarquia abaixo valem também para `policy.transaction-costs`.
  - Hierarquia de fontes: (1) contrato ou proposta vinculante assinada, com a validade do documento; (2) tabela pública oficial vigente na data da operação (CVM, ANBIMA, B3, emolumentos do estado do cartório), até a próxima versão; (3) cotação escrita e datada do prestador, válida por 90 dias; (4) provisão da casa de `policy.transaction-costs`, em faixa por instrumento e rotulada estimativa, com a validade da versão aprovada daquela chave, só para leitura preliminar e nunca em material externo.
...
- **Regra de aplicação.**
  ...
  6. Exemplos com números públicos:
     ...
     - Nota comercial bullet de R$ 20 milhões a profissionais, 2 anos: CVM R$ 6.000; ANBIMA R$ 9.919 (a alíquota daria R$ 555,60); total de R$ 15.919, ou 7,96 pontos-base e 3,98 pontos-base ao ano.
     - CCB bullet de 1 ano ou mais a pessoa jurídica: IOF de 3,373% do principal, o teto de `policy.capital.iof`, de onde vem o valor que entra no cálculo; em prazo médio de 3 anos, 112,4 pontos-base ao ano pelo método linear.
  7. Executor atual: `buildPricingTruthSet` recebe `costs` com `oneTimeAmount`, `annualAmount`, `sourceId` e `validUntil`, rejeita componente vencido, anualiza pelo método linear e, desde 24/09/2026, compõe o all-in com o CDI pela fórmula desta chave; a TIR ainda não é calculada pelo executor.
- **Fundamento.** PR-10 manda somar estruturação, registro, garantias, monitoria e jurídico ao spread, anualizar no prazo médio e comparar instrumentos em all-in. O método linear é o que o executor lê e é o jeito de mesa de explicar custo; a TIR é o custo econômico e governa quando os dois se afastam. As taxas regulatórias são pequenas frente aos custos de coordenação, jurídico, agente fiduciário e rating, que variam por operação e por isso vêm de contrato ou cotação; o catálogo não publica faixa desses custos sem cotação datada. O IOF faz a CCB carregar até 3,373% do principal quando o prazo passa de um ano, enquanto a debênture carrega custos fixos de emissão que pesam mais no tíquete pequeno; a escolha entre os dois só se faz em all-in. O regime vigente do IOF, com a situação da ADC 96 no STF, é o de `policy.capital.iof`, e o tratamento tributário de papéis isentos é o de `policy.capital.tax-regime`. A quarta fonte da hierarquia é a provisão de `policy.transaction-costs` porque é a única faixa de custo sem cotação que a casa publica; o catálogo não mantém referência de mercado própria.
- **Fontes.**
...
- **Revisão.** Nova versão de qualquer tabela pública citada; nova versão de `policy.capital.iof` ou de `policy.transaction-costs`; mudança legal sobre agente fiduciário, escrituração ou taxa da CVM; alinhamento do executor à TIR.
- **Estado.** Preparado pela Offroad em 24/09/2026; aguardando revisão do fundador.
...
- **Valor proposto.** Registro governado de mandatos, sem instituição incluída nesta proposta, com estas regras:
  ...
  - Precedência por campo: para restrição legal ou regulamentar (instrumentos, setores vedados, geografias), regulamento publicado, depois confirmação direta, observação e inferência; para apetite (ativo, tíquete, prazo, setores, garantia, alavancagem, cobertura, indexadores, faixa de risco, retorno), confirmação direta, observação, regulamento e inferência. Declaração e conversa formam a mesma classe de confirmação direta, e entre as duas prevalece a mais recente.
...
- **Regra de aplicação.**
  ...
  6. Executor atual: `buildMarketTruthSet` mapeia `declared` e `conversation` para confirmação direta, `published` para regra pública, `observed` para observação governada e o resto para não confirmado, e bloqueia o destinatário com campo duro ausente, vencido, não confirmado ou divergente. Desde 24/09/2026 o resolvedor de `packages/fund-mandate` aplica a precedência por campo acima: instrumentos e geografias pela ordem de restrição legal, os demais campos pela ordem de apetite, com declaração e conversa na mesma classe e a mais recente prevalecendo. Setores vedados, indexadores, faixa de risco e retorno alvo ainda não são campos do modelo de mandato daquele pacote.
...
- **Valor proposto.**
  ...
  - Reconfirmação antes da onda: ativo e tíquete confirmados diretamente (declaração ou conversa) nos últimos 30 dias para qualquer destinatário de uma onda (MK-17).
