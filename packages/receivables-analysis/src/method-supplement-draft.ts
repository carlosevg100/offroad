...
export const receivablesSupplementPatchSchema = z.object({
  ...
  if (Object.keys(patch.sections).length === 0) {
...
export function compileReceivablesSupplementDraft(input: unknown): ReceivablesSupplementDraftStatus {
  ...
  const requiredSections: SectionName[] = ["cedent", "titles", "cashReceipts", "accounting", "policy", "structure"];
  ...
  if (openConflictIds.length > 0) return {state: "conflicted", missingSections, openConflictIds, supplement: null};
  if (missingSections.length > 0) return {state: "incomplete", missingSections, openConflictIds, supplement: null};
  ...
  const supplement = receivablesPoolInputSupplementSchema.parse({
    ...
    policy: draft.sections.policy!.value,
    structure: draft.sections.structure!.value,
