...
export function buildCapitalPeriodCash(input: CapitalPeriodCashInput) {
  ...
  const common = {schemaVersion: "capital-period-cash.v1" as const, engineVersion: capitalPeriodCashVersion,
    operands: structuredClone(input), operating: op, financing,
  ...
  const rows = op.rows.map(period => {
    ...
    cash = cash!.plus(period.cashBeforeFinancing).plus(availableFinance);
    restricted = restricted!.plus(restrictedFinance);
    ...
    return {periodId: period.periodId, startDate: period.startDate, endDate: period.endDate,
      ...
      netFinancingRestricted: restrictedFinance.toFixed(), closingAvailable: cash.toFixed(), closingRestricted: restricted.toFixed(),
