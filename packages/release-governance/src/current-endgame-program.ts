...
const tasks: ProgramTask[] = [
  ...
  task({taskId: "RT-04", releaseId: "R1", state: "in_progress", title: "Compositor de especializações", outcome: "Intenção e caracterização aberta do negócio compõem núcleo financeiro, mecanismos econômicos e expertise específica por objeto, instrumento e jurisdição.", ownerRole: "DCM specialization engineer", dependsOn: ["RT-01", "RT-03"], steps: ["Caracterizar atividades e objetos sem exigir setor catalogado", "Compor mecanismos e métodos com precedência e aplicabilidade comprovada", "Preservar segmentos, entidades e perímetros de consolidação", "Resolver conflitos e requisitos por tarefa e dependência", "Bloquear conclusões sem suporte e preservar trabalho independente autorizado"], acceptance: ["Composição é determinística e vinculada à intenção e às evidências", "Ausência de rótulo setorial não exclui a companhia", "Método não é autorizado por analogia ou narrativa", "Lacunas bloqueiam tarefas e conclusões dependentes sem liberar entregas finais materialmente incompletas"], capabilityRefs: ["workflow.composable-specialization-shadow", "workflow.specialist-method-binding-shadow"], blueprintRefs: [`${B}#20.3-release-1---objective-to-plan-universal`, `${B}#20.7-release-5---generalidade-econômica`], securityControlIds: ["TRUST-AI-01"]}),
  ...
  task({taskId: "WFI-13", releaseId: "R5", state: "in_progress", title: "Cobertura aberta de negócios e métodos específicos", outcome: "Qualquer negócio no mandato da Offroad pode ser caracterizado; a profundidade comprovada é composta por intenção, atividade, mecanismo econômico e objeto, sem lista setorial de elegibilidade.", ownerRole: "Workflow intelligence engineer", dependsOn: ["WFI-01", "RT-07", "RT-04"], steps: ["Definir caracterização extensível com fontes, atributos confirmados, inferidos e desconhecidos", "Vincular intenção e resultado a atividades, objetos e perímetros relevantes", "Compor núcleo financeiro, mecanismos e métodos específicos com aplicabilidade revisada", "Tratar negócios mistos, entidades, segmentos e consolidação", "Vincular dados, método, cálculo, executor e prontidão a cada tarefa", "Expandir métodos conforme necessidades e riscos materiais observados", "Validar negócios não usados no desenvolvimento e combinações inéditas", "Verificar tarefas pontuais, jornadas e diferentes perfis no mesmo negócio", "Propagar nuances para documentos, reconciliação, cenários, materiais e matching", "Submeter novos métodos e correções a revisão e gates antes de promoção"], acceptance: ["Negócio não catalogado recebe caracterização e plano sem elegibilidade por lista fechada", "Mesma companhia recebe profundidade adequada a cada intenção", "Atividades heterogêneas mantêm perímetros explícitos e consolidação justificada", "Lacuna específica impede conclusão dependente e mantém trabalho independente autorizado", "Nenhuma analogia ou correção isolada promove método ou executor", "Amostras por mecanismos, materialidade e risco se expandem sem virar limite comercial", "Gold, adversarial, consistency e revisão comprovam apenas o escopo declarado"], blueprintRefs: [`${B}#8.3.1-arquitetura-para-qualquer-negócio`, `${B}#20.7-release-5---generalidade-econômica`], securityControlIds: ["TRUST-AI-01", "TRUST-AI-02"]}),
  task({taskId: "WFI-14", releaseId: "R5", title: "Gate agregado do specialist runtime", outcome: "O runtime só avança no escopo de métodos comprovado após gates individuais e integrados; essa promoção não certifica todos os negócios nem cria lista setorial de elegibilidade.", ownerRole: "Workflow intelligence lead", dependsOn: ["WFI-01", "WFI-02", "WFI-03", "WFI-04", "WFI-05", "WFI-06", "WFI-07", "WFI-08", "WFI-09", "WFI-10", "WFI-11", "WFI-12", "WFI-13"], steps: ["Verificar gates de todos os métodos e packs requeridos pelo escopo declarado", "Executar composição, conflitos e negócios com atividades heterogêneas", "Avaliar negócios não usados no desenvolvimento e mudanças de intenção", "Medir bloqueio de conclusões sem suporte e execução independente autorizada", "Registrar acreditação agregada com limites explícitos"], acceptance: ["Nenhum pack isolado promove o runtime geral", "Todos os packs requeridos estão gate_passed", "Composição sem suporte bloqueia conclusões dependentes e entregas finais com lacuna material", "Negócios inéditos e mistos são avaliados sem promover expertise universal nem restringir elegibilidade por setor"], blueprintRefs: [`${B}#20.7-release-5---generalidade-econômica`], securityControlIds: ["TRUST-AI-01", "TRUST-AI-02"], capabilityTransition: {capabilityId: "execution.general-specialist-runtime", from: "specified", to: "tested", status: "planned", evidenceRefs: []}}),
...
export const currentEndgameProgramBoard: EndgameProgramBoard = {
  boardVersion: "2026.09.09-v5",
  generatedAt: "2026-09-09T07:20:19.652-03:00",
  baseline: {
  ...
  evidenceIndex: [
    ...
    {evidenceId: "EV-SEC01-INVENTORY", kind: "document", ref: "docs/security/history/wave-1-initial-inventory.md", environment: "repository", capturedAt: "2026-09-14T21:45:33.165Z", validThrough: null, description: "Snapshot documental imutável da revisão wave-1: 41 evidências e 21 lacunas; não atesta estado operacional corrente nem fechamento de lacunas. O gate confiável do inventário continua independente e obrigatório na CI.", immutableFingerprint: "c4dd3d59b58e11dc2be0865394a7ce7f92b696aff5d20e06c1d215f960d5be44"},
