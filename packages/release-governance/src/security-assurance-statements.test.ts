...
function signedEvidence(overrides: Partial<Omit<SecurityAssuranceEvidence, "detachedSignature">> = {}): SecurityAssuranceEvidence {
  const unsigned = {
    ...
    contentFingerprint: `sha256:${"b".repeat(64)}` as const,
    ...overrides,
  };
...
describe("governed security assurance statements", () => {
  it("renders an attested claim only from current, scope-matched, signed evidence under a trusted root", () => {
    ...
    const decision = evaluateSecurityAssuranceStatementAgainstTrustedRoots({statement, evidence: [signedEvidence()], trustedRoots: [root], evaluatedAt: now});
  ...
  it("rejects a tampered attestation and a reconstructed render decision", () => {
    ...
    const decision = evaluateSecurityAssuranceStatementAgainstTrustedRoots({statement, evidence: [evidence], trustedRoots: [root], evaluatedAt: now});
    expect(decision.allowed).toBe(false);
  ...
  it.each([
    ...
    const decision = evaluateSecurityAssuranceStatementAgainstTrustedRoots({statement, evidence: [], trustedRoots: [], evaluatedAt: now});
    expect(decision.allowed).toBe(true);
