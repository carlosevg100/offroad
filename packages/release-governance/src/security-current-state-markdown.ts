...
export function renderSecurityCurrentStateInventory(
  ...
  const lines: string[] = [
    ...
    "",
    `Baseline: \`${inventory.baseline.repository}@${inventory.baseline.commit}\`, evidência até ${inventory.baseline.evidenceCutoff}.`,
    "",
    `Revisão por onda: \`${inventory.baseline.waveId}\`; estado \`${inventory.baseline.waveStatus}\`; mudanças materiais \`${inventory.baseline.materialChangeState}\`.${inventory.baseline.reviewDueAt ? ` Prazo adicional: ${inventory.baseline.reviewDueAt}.` : " Revisar no fechamento e antes de mudanças materiais."}`,
    "",
