...
describe("from the book to the paper", () => {
  ...
  it("turns the allocated lines into terms, each with its own pricing, cut to what the book gave", () => {
    ...
    // CDI plus spread compounds: at a 10,50% CDI, CDI + 3,90% costs 14,81% and CDI + 4,40% costs
    // 15,36%, so the fixed 15,00% indication is the second cheapest and fills the book before it.
    expect(terms.lines).toHaveLength(2);
    expect(terms.total).toBe("42300000");
    expect(terms.lines.map((line) => line.terms.principal)).toEqual(["20000000", "22300000"]);
    expect(terms.lines[0]!.conditions).toEqual(["conta vinculada"]);
    expect(terms.lines[1]!.firm).toBe(false);
    expect(terms.weightedAllInPct).toBe("14.91");
  ...
  it("consolidates the schedules on the calendar and names the peak month", () => {
    ...
    expect(schedule.perInvestor).toHaveLength(2);
