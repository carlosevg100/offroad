...
try {
  ...
  await build({stdin: {contents: `
    export {buildCapitalProcedureRuns} from './packages/financial-model/src/capital-procedure-runs.test-support';
  ...
  const {buildCapitalProcedureRuns, deterministicMethodRunSchema} = await import(pathToFileURL(outfile));
  ...
  const records = buildCapitalProcedureRuns().map(evidence => deterministicMethodRunSchema.parse({
