...
Sujeito: método `{{METHOD_ID}}` versão {{METHOD_VERSION}} em `{{METHOD_PATH}}`; executor em
`{{EXECUTOR_PATH}}`; testes em `{{TEST_PATH}}`. Contrato: `packages/credit-playbook/src/procedure-contract.ts`.
Fontes do caso gold: `{{ANSWER_KEY_PATH}}` e o corpus em `{{CORPUS_DIR}}` (manifesto com hashes).
