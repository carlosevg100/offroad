...
export function renderMarkdownReport(report: EvalReport, options: {thresholds?: Thresholds} = {}): string {
  ...
  lines.push(`| All fields recall | ${pct(report.fields.all.recall)} (${report.fields.all.matched}/${report.fields.all.expected}) | — | — |`);
  ...
  lines.push(`| Unscored candidates (fields not in gold) | ${report.fields.unscoredCandidates} | — | — |`);
  ...
  lines.push(`| Exception recall | ${pct(report.exceptions.recall)} · false positives ${report.exceptions.falsePositives}/${report.exceptions.produced} | ≥ ${pct(thresholds.exceptionRecall)} | ${report.exceptions.recall === null ? "n/a" : flag(verdicts.exceptionRecall)} |`);
  lines.push(`| Calculations matched | ${pct(report.calculations.recall)} (${report.calculations.matched}/${report.calculations.expected}) | — | — |`);
  lines.push(`| Acceptance weight (evaluated) | ${report.acceptance.passedWeight}/${report.acceptance.totalWeight} · critical failures: ${report.acceptance.criticalFailures.join(", ") || "none"} | — | — |`);
  if (report.usage) lines.push(`| Cost / calls | US$ ${report.usage.costUsd.toFixed(4)} · ${report.usage.calls} calls | — | — |`);
  lines.push("");
  lines.push("## Fields");
  lines.push("");
  ...
  for (const outcome of report.fields.outcomes) {
    lines.push(`| ${outcome.fieldPath}${outcome.periodEnd ? ` (${outcome.periodEnd})` : ""} | ${outcome.materiality} | ${outcome.expected} | ${outcome.found ?? "—"} | ${outcome.status} |`);
  }
  lines.push("");
  ...
  lines.push("|---|---|---|---|");
  for (const outcome of report.exceptions.outcomes) lines.push(`| ${outcome.id} | ${outcome.severity} | ${outcome.status} | ${outcome.matchedBy ?? "—"} |`);
  lines.push("");
  lines.push("## Acceptance criteria");
  lines.push("");
  lines.push("| Id | Weight | Critical | Status | Reasons |");
  lines.push("|---|---|---|---|---|");
  for (const outcome of report.acceptance.outcomes) lines.push(`| ${outcome.id} | ${outcome.weight} | ${outcome.critical ? "yes" : "no"} | ${outcome.status} | ${outcome.reasons.join("; ") || "—"} |`);
  lines.push("");
  return lines.join("\n");
}
