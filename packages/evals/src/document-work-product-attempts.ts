...
export function summarizeDocumentWorkAttempts(
  ...
  const firstPassSuccessCount = runs.filter(run=>run.passed && run.narrativeCalls === 1 && run.firstResponseValid).length;
  const bounded = runs.every(run=>run.completeCalls >= 2 && run.completeCalls <= 3 && run.narrativeCalls >= 1 && run.narrativeCalls <= 2
    && run.reviewCalls === 1 && run.completeCalls === run.narrativeCalls + run.reviewCalls && run.providerCalls >= run.completeCalls)
