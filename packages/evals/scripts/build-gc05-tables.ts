...
// 3. Alternatives on the same model (executors of case 01) and the turn-3 bridge.
...
const exit = executors.estimateExitCostBySeries({exitDate: "2026-09-04", unit: "BRL thousand", series: [
  ...
  {id: "deb-11", label: "11ª emissão", principal: "657779", rule: {mechanism: "redemption_offer", premiumPercent: null, requiresFullAdherence: true, availableFrom: null}, anchor: {document: "escritura_11a_emissao.pdf", clause: "4.14"}},
]});
const premium = (id: string) => exit.exitCosts.find((entry) => entry.seriesId === id)!.premium;
const beforeAfter = (rate: string, termMonths: number) => executors.compareRefinancingBeforeAfter({
  referenceDate: "2026-09-04", unit: "BRL thousand",
  before: {grossDebt: "5670186", unrestrictedCash: "1455809", derivativeLiabilities: "14335", derivativeAssets: "235", ltmEbitda: {value: "895864", basis: "implied from 4,72x (derived)"}, schedule: {"2027": "1229828", "2028": "776868", "2029": "1228475", "2030": "694497", "2031": "994544", "2032+": "809198"}, weightedAverageRate: "0.1246", anchor: {document: "01_ITR_1T26_31mai2026.pdf", page: 40, note: "15"}},
  ...
  alternatives: [
    ...
    {id: "x-extend-di-2028", label: "X: alongar a parede de 2028/29 retirando a 13ª 1ª série", newDebt: {amount: "306038", annualRate: rate, termMonths, graceMonths: 24, format: "sac", upfrontFeeRate: "0.005", origin: "custo de referência do pedido simulado (CDI + 1,25%), sintético", anchor: {document: "03_Pedido_Simulado_CRA_2026.docx", page: 1}}, retired: [{seriesId: "deb-13-1", principal: "306038", exitPremium: premium("deb-13-1"), maturityPeriod: "2029"}]},
    {id: "y-extend-both", label: "Y: alongar as duas paredes retirando a 13ª 1ª e a 14ª 1ª séries", newDebt: {amount: "744956", annualRate: rate, termMonths, graceMonths: 24, format: "sac", upfrontFeeRate: "0.005", origin: "idem", anchor: {document: "03_Pedido_Simulado_CRA_2026.docx", page: 1}}, retired: [{seriesId: "deb-13-1", principal: "306038", exitPremium: premium("deb-13-1"), maturityPeriod: "2029"}, {seriesId: "deb-14-1", principal: "438918", exitPremium: premium("deb-14-1"), maturityPeriod: "2030"}]},
    {id: "offer-11th", label: "Retirar a 11ª por oferta (prêmio a negociar)", newDebt: null, retired: [{seriesId: "deb-11", principal: "657779", exitPremium: premium("deb-11"), maturityPeriod: "2029"}]},
  ],
...
const line = (result: ReturnType<typeof beforeAfter>, id: string) => { const a = result.alternatives.find((alternative) => alternative.id === id)!; return a.state === "blocked" ? `blocked: ${a.blockReasons.join("; ")}` : `custo de saída ${fmt(a.exitCost)}; pico ${a.after!.peak?.period} ${fmt(a.after!.peak?.amount)}; all-in ${a.after!.allInCost ? pct(a.after!.allInCost) : "n/a"}; 2029 ${fmt(a.concentration!.find((row) => row.period === "2029")?.consolidated)}; 2030 ${fmt(a.concentration!.find((row) => row.period === "2030")?.consolidated)}`; };
...
md.push("", `Ranking turno 1: ${turn1.ranking!.order.map((entry) => `${entry.id} (${fmt(d(entry.value).negated())})`).join(" > ")}. Ranking turno 3: ${turn3.ranking!.order.map((entry) => `${entry.id} (${fmt(d(entry.value).negated())})`).join(" > ")}.`);
md.push("", "### Bridge do turno 3 (o que foi recomputado e o que não foi)", "", "| Nó | Turno 1 | Turno 3 | Recomputado |", "| --- | --- | --- | --- |",
  ...
  `| serviço da nova dívida de X | ${fmt(turn1.alternatives.find((a) => a.id === "x-extend-di-2028")!.newDebtService?.totalInterest)} de juros | ${fmt(turn3.alternatives.find((a) => a.id === "x-extend-di-2028")!.newDebtService?.totalInterest)} de juros | sim |`,
  `| all-in de X | ${pct(turn1.alternatives.find((a) => a.id === "x-extend-di-2028")!.after!.allInCost)} | ${pct(turn3.alternatives.find((a) => a.id === "x-extend-di-2028")!.after!.allInCost)} | sim |`,
  `| concentração depois de X | 2029 ${fmt(turn1.alternatives.find((a) => a.id === "x-extend-di-2028")!.concentration!.find((row) => row.period === "2029")?.consolidated)} | 2029 ${fmt(turn3.alternatives.find((a) => a.id === "x-extend-di-2028")!.concentration!.find((row) => row.period === "2029")?.consolidated)} | sim |`,
  `| ranking | ${turn1.ranking!.order.map((entry) => entry.id).join(" > ")} | ${turn3.ranking!.order.map((entry) => entry.id).join(" > ")} | ${turn1.ranking!.order.map((entry) => entry.id).join() === turn3.ranking!.order.map((entry) => entry.id).join() ? "recomputado, sem mudança de ordem" : "recomputado, ordem mudou"} |`,
