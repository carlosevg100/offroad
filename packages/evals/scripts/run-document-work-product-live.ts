...
async function main() {
  ...
  // Fixed partitions retain an aggregate USD3 ceiling, including the seven reviewer controls.
  ...
  const controlGateway=createModelGateway({adapters,budget:{maxCostUsd:0.5,maxCalls:7},onCall:call=>controlCalls.push(call)});
