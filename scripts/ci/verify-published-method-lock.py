...
def verify(previous,current,identity_key="platformReleaseId"):
 before={r[identity_key]:r for r in previous['releases']}
 after={r[identity_key]:r for r in current['releases']}
...
if __name__=='__main__':
 paths=[path,'packages/credit-playbook/knowledge/releases/compiled-executor-lock.json','apps/document-worker/preparers/preparer-lock.json']
 for path in paths:
  ...
  identity_key="id" if current.get("schemaVersion")=="technical-preparer-lock.v1" else "platformReleaseId"
  verify(previous,current,identity_key)
  for release in current['releases']:
   ...
   if not any(r[identity_key]==release[identity_key] for r in previous['releases']):
