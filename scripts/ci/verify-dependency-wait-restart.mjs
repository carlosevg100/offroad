// Throwaway CI database only: every write here commits on the local stack of the Database job, which
// rebuilds it from the migrations right before this step, and nowhere else. Stage 18, increment 6B: a
// human wait survives a restart of the worker. A synthetic capital work carries a governed working
// basis; the released capital method is installed with the profile its manifest derives, released and
// universal, except that this profile can spend: a ceiling above zero, which the storage check refuses
// for every stored profile, so it exists only here, inserted with that check off for this one row. The
// capital method admits exactly one executable profile and one requestable release, so this proof
// cannot share a database with scripts/ci/verify-dependency-recompute.mjs (the zero-budget proof of
// 3B) and refuses to start on a database that already has a capital release or a recompute candidate.
// The founder grants the organization's producer. The owner, signed in, requests the root execution
// through request_work_execution_v2 with the composition the web action uses. A new revision of the
// working basis changes one decision, and the dependency effect of its events plans one candidate
// that, under the costed profile, waits for a person: an awaiting_human milestone, no job, no lease.
// The worker account, signed in and bound to its worker token, runs the recompute loop until idle; a
// fresh client for another worker account bound to its own token (the restarted worker, a new lease
// owner) does too; the wait and the candidate are unchanged and nothing was claimed. The owner
// authorizes through authorize_work_update_v1, and the restarted loop produces exactly one execution,
// for the original requester, pinning the head revision, with a gate receipt, a queued job, lineage
// naming the root and the wait resolved once by an approved decision. The recompute health the worker
// reads (6A) counts the wait before the loop and the scheduled candidate after the authorization. No
// model, no provider key and no network beyond the local stack; synthetic data only.
...
const temporary = mkdtempSync(join(tmpdir(), 'offroad-dependency-wait-restart-proof-'));
...
export {releasedMethodArtifacts} from './src/released-methods.generated.ts';`, resolveDir: join(root, 'apps/document-worker'), loader: 'ts'},
  ...
  // The rebuilt database: no capital release and no recompute candidate before this proof.
  assert.equal(sql(`select (select count(*) from private.platform_method_releases where method_id='${purpose}')+(select count(*) from public.work_recompute_candidates);`), '0',
...
commit;`);
  ...
  // The rebuild restarts the auth and API services: a sign-in that finds them still starting is
  // retried, and only that.
  const session = async (email, headers = {}) => {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      if (error && (error.status === undefined || error.status === 0 || error.status >= 500) && attempt < 30) { await new Promise(done => setTimeout(done, 1000)); continue; }
...
commit;`);
 ...
 'decisions',(select jsonb_agg(m.outcome) from public.work_milestones m where m.kind='decision' and m.subject_kind='work_recompute_candidate' and m.subject_id='${candidateId}'));`));
  ...
  console.log(JSON.stringify({event: 'dependency_wait_restart_eval', pinnedHypotheses: governed.pins.length, outcomes: outcomes.map(outcome => outcome.status), loopMs,
    proofMs: Math.ceil(performance.now() - proofStarted)}));
  console.log('dependency_wait_across_restart: PASS (a costed candidate waits as an awaiting_human milestone with no job and no lease through the running loop and a restarted worker with a new lease owner; wait and candidate unchanged, nothing claimed; authorize_work_update_v1 by the owner; one production by the restarted worker for the original requester, head revision pinned, gate receipt, queued job, lineage; the wait resolved once by an approved decision; recompute.health counted the wait, then the scheduled candidate; rebuilt throwaway local stack)');
