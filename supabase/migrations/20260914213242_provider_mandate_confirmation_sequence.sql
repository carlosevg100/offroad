...
--
-- Retires the clock_timestamp() default applied on 11 September 2026 (migration
-- provider_mandate_confirmation_clock_timestamp). A value later than the transaction's now()
-- made a confirmation invisible to a same-transaction reader that asks for the mandate "as of
-- now()", which is exactly what the case-fit projection does. confirmed_at returns to the
