-- Stage 10: specialized historical starts also resolve work identity, never fictional intake.
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended((select organization_id::text from private.workspace_membership_v1())||p_request_id::text,0));
  ...
  where brief.organization_id=(select organization_id from private.workspace_membership_v1())
    and brief.request_id = p_request_id
  ...
  if found then
    ...
    if existing_brief.created_by is distinct from caller_id then raise exception 'work_request_denied' using errcode='42501'; end if;
    if existing_brief.content is distinct from p_brief then raise exception 'work_request_replay_conflict' using errcode='22023'; end if;
    ...
    if v_session_id is null then
      ...
      return work_result||jsonb_build_object('capital_project_id',existing_brief.capital_project_id,'intake_session_id',null,'brief_id',existing_brief.id,'job_id',work_result->>'jobId','executionState','conversation_only','replayed',true);
    end if;
  ...
  if length(btrim(coalesce(p_company_name,''))) not between 2 and 160
    or length(coalesce(p_company_website,''))>500
    or (nullif(btrim(p_company_website),'') is not null and p_company_website !~* '^https?://[^[:space:]]+$') then raise exception 'invalid_public_project_setup' using errcode='22023'; end if;
  work_prompt:=concat_ws(E'\n',btrim(p_project_name),btrim(p_company_name),(select string_agg(value,E'\n' order by key) from jsonb_each_text(p_brief)));
  work_result:=private.start_work_v1(p_request_id,p_locale,p_project_name,work_prompt,'company_debt_view','public_information',p_plan,null,false);
  v_project_id:=(work_result->>'workId')::uuid;
  v_session_id:=(work_result->>'intake_session_id')::uuid;
  update public.dossiers set profile=jsonb_strip_nulls(jsonb_build_object('name',btrim(p_company_name),'website',nullif(btrim(p_company_website),'')))
    where resource_id=v_project_id and organization_id=(select organization_id from private.workspace_membership_v1());
  perform private.link_work_dossier_v1(v_project_id,(select id from public.dossiers where resource_id=v_project_id and organization_id=(select organization_id from private.workspace_membership_v1())));
  ...
  if v_session_id is null then
    work_result:=work_result||private.enqueue_work_turn_v1(v_project_id,p_request_id);
    return work_result||jsonb_build_object('intake_session_id',null,'brief_id',v_brief_id,'job_id',work_result->>'jobId','executionState','conversation_only');
  end if;
...
begin
  ...
  if length(btrim(coalesce(p_company_name,''))) not between 2 and 160
    ...
    or (nullif(btrim(p_company_website),'') is not null and p_company_website !~* '^https?://[^[:space:]]+$') then raise exception 'invalid_public_project_setup' using errcode='22023'; end if;
  work_prompt:=concat_ws(E'\n',btrim(p_project_name),btrim(p_company_name),(select string_agg(value,E'\n' order by key) from jsonb_each_text(p_brief)));
  work_result:=private.start_work_v1(p_request_id,p_locale,p_project_name,work_prompt,'origination_thesis','public_information',p_plan,null,false);
  v_project_id:=(work_result->>'workId')::uuid;
  v_session_id:=(work_result->>'intake_session_id')::uuid;
  update public.dossiers set profile=jsonb_strip_nulls(jsonb_build_object('name',btrim(p_company_name),'website',nullif(btrim(p_company_website),'')))
    where resource_id=v_project_id and organization_id=(select organization_id from private.workspace_membership_v1());
