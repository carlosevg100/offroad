-- integration_preview: the preview run does not require an artifact per task run.
--
-- Each turn compiles its own plan and unchanged steps replay by input fingerprint: the replay is a
-- run of the turn's plan whose output points at the object recorded by an earlier plan, and no new
-- artifact is written for it. The task-run trigger requires an artifact owned by every succeeded
-- run when the job carries capital_artifact_required = true, so the preview run carries false.
-- Computed steps still record their artifacts (the completion binds the readout to the last one),
-- and production DAGs keep the requirement. Only the job payload of the activation changes.
