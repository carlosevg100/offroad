-- Turn the confirmed conversational first read into the canonical intake event stream.
-- The worker may propose identity and a request route, but neither becomes project memory until
-- the user confirms the exact fingerprint. The transaction records the declaration, route,
-- company projection and gate decision together, so the tailored request ladder never starts
-- from an empty legacy wizard state.
...
create or replace function public.decide_advisor_preliminary_v1(
  p_project_id uuid,
  p_object_fingerprint text,
  p_decision text,
  p_correction text default null
...
set search_path = ''
as $$
  select private.decide_advisor_preliminary_v1(
    p_project_id, p_object_fingerprint, p_decision, p_correction
  );
$$;

revoke all on function private.decide_advisor_preliminary_v1(uuid, text, text, text)
  from public, anon, authenticated;
revoke all on function public.decide_advisor_preliminary_v1(uuid, text, text, text)
  from public, anon;
grant execute on function private.decide_advisor_preliminary_v1(uuid, text, text, text)
  to authenticated;
grant execute on function public.decide_advisor_preliminary_v1(uuid, text, text, text)
  to authenticated;

comment on function public.decide_advisor_preliminary_v1(uuid, text, text, text) is
  'Confirms or corrects the current conversational first read. Confirmation atomically seeds canonical intake memory before tailored requests are compiled.';
