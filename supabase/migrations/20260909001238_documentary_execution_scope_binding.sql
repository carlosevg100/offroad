...
returns jsonb language sql stable security definer set search_path='' as $$
 ...
 || case when b.internal_snapshot->>'planVersion' like 'document-work-plan.v1:%'
   and (select array_agg(t.value order by t.value)
        from jsonb_array_elements(b.internal_snapshot->'workstreams') w(value)
        cross join lateral jsonb_array_elements_text(w.value->'sourceTaskIds') t(value)) = array['Q01','Q02','Q03']::text[]
   and (select array_agg(t.value->>'id' order by t.value->>'id') from jsonb_array_elements(p.snapshot->'taskSpecs') t(value)) = array['Q01','Q02','Q03']::text[]
