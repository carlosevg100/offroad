-- Extend the existing scoped reader to every canonical economic dimension.
-- No new data store, grant, inferred applicability or financial method authority.
...
begin
  ...
  where c.organization_id=p_organization_id and c.intake_session_id=p_session_id
    and c.field_path in ('company.sector','company.subsector','company.business_model','company.revenue_model','company.lifecycle','company.recourse','company.jurisdiction','company.cost_model','company.working_capital','company.asset_model','company.capital_expenditure','company.regulation','company.operating_driver')
