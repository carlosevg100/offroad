-- Cache v2 invalidates fallback-only records written before authoritative-provider failures were
-- excluded from reusable public raw material. V1 remains stored for audit/expiry, but workers load
-- and write only v2. Query ids retain the independently verifiable sha256(topic:query) contract.

alter table private.public_research_query_cache
  drop constraint public_research_query_cache_schema_check;

alter table private.public_research_query_cache
  add constraint public_research_query_cache_schema_check
  check (schema_version in ('public-research-cache.v1', 'public-research-cache.v2'));
...
comment on table private.public_research_query_cache is
  'Global cache of public raw research only. Workers use v2, which excludes fallback-only material after authoritative-source failures; v1 is retained only until expiry.';
