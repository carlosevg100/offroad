...
-- Recorded statements MD5 (joined with newline): f12ff028ff9fd0a0de1ccead0d9f270e
...
declare
  ...
  request_hash text := encode(extensions.digest(convert_to(p_suggestions::text, 'UTF8'), 'sha256'), 'hex');
...
begin
  ...
  if found then
    if existing.intake_session_id is distinct from job_row.intake_session_id
      or existing.event_type <> 'analysis_scope_suggestions_recorded'
      or existing.payload ->> 'requestHash' is distinct from request_hash then
      raise exception 'intake_event_idempotency_conflict' using errcode = '23505';
    end if;
  ...
  event_row := private.append_intake_domain_event(
    ...
    'analysis_scope_suggestions_recorded', jsonb_build_object(
...
declare
  ...
  request_hash text := encode(extensions.digest(
    convert_to(job_row.source_document_id::text, 'UTF8'), 'sha256'
  ), 'hex');
  actor_id uuid := (select auth.uid());
  occurred_at timestamptz := clock_timestamp();
begin
  ...
  event_row := private.append_intake_domain_event(
    job_row.organization_id, job_row.intake_session_id, p_event_id,
    'advisor_authorization_recorded', jsonb_build_object(
...
begin
  ...
  request_hash := encode(extensions.digest(convert_to(jsonb_build_object(
    'suggestionId', trim(p_suggestion_id),
    'decision', p_decision,
    'role', p_role,
    'reason', trim(p_reason),
    'scopeEventId', p_scope_event_id
  ...
  suggestion_event := private.append_intake_domain_event(
    ...
    jsonb_build_object('suggestions', next_suggestions, 'requestHash', request_hash), occurred_at, actor_id
  );
...
begin
  ...
  request_hash := encode(extensions.digest(convert_to(jsonb_build_object(
    'reason', trim(p_reason), 'transition', 'revoked'
  )::text, 'UTF8'), 'sha256'), 'hex');
  select * into existing
  from public.intake_domain_events event
  where event.organization_id = p_organization_id and event.event_id = p_event_id;
  ...
  event_row := private.append_intake_domain_event(
    ...
    jsonb_build_object('authorization', authorization_value, 'requestHash', request_hash), occurred_at, actor_id
  );
...
begin
  ...
  request_hash := encode(extensions.digest(convert_to(jsonb_build_object(
    'reason', trim(p_reason), 'transition', 'verified'
  )::text, 'UTF8'), 'sha256'), 'hex');
  select * into existing
  from public.intake_domain_events event
  where event.organization_id = p_organization_id and event.event_id = p_event_id;
