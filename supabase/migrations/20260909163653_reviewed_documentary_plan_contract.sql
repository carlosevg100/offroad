-- Preserve exact previously approved v9/v10 plans; admit the new reviewed-revision compiler.
create or replace function private.is_released_documentary_plan_v10(p_snapshot jsonb, p_entry text)
...
$function$;
revoke all on function private.is_released_documentary_plan_v10(jsonb,text) from public,anon,authenticated;

create or replace function private.is_released_documentary_plan_v1(p_snapshot jsonb, p_entry text)
returns boolean language sql immutable security invoker set search_path='' as $function$
  select private.is_released_documentary_plan_v10(p_snapshot,p_entry) or coalesce(
    p_entry in ('structure_from_documents','review_existing_operation')
