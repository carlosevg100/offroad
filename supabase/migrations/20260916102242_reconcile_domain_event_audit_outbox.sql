-- Reconciles the preserved staging application 20260916101915.
-- Safe on its installed schema and on a fresh production/replay schema.
set local lock_timeout = '5s';
-- Durable authority events; no historical audit is relabelled as a new event.
do $constraint$ begin if not exists(select 1 from pg_constraint where conrelid='public.audit_events'::regclass and conname='audit_events_organization_id_id_key') then alter table public.audit_events add constraint audit_events_organization_id_id_key unique (organization_id,id); end if; end $constraint$;
do $constraint$ begin if not exists(select 1 from pg_constraint where conrelid='private.human_intervention_ledger'::regclass and conname='human_intervention_ledger_organization_id_id_key') then alter table private.human_intervention_ledger add constraint human_intervention_ledger_organization_id_id_key unique(organization_id,id); end if; end $constraint$;
do $constraint$ begin if not exists(select 1 from pg_constraint where conrelid='private.retrieval_audit_events'::regclass and conname='retrieval_audit_events_organization_id_id_key') then alter table private.retrieval_audit_events add constraint retrieval_audit_events_organization_id_id_key unique(organization_id,id); end if; end $constraint$;
alter table private.worker_tokens add column if not exists event_outbox_polled_at timestamptz;
...
create index if not exists domain_events_audit_idx on private.domain_events(organization_id,audit_event_id);
create index if not exists domain_events_human_idx on private.domain_events(organization_id,human_intervention_id);
create index if not exists domain_events_retrieval_idx on private.domain_events(organization_id,retrieval_audit_event_id);
...
create index if not exists event_outbox_ready_idx on private.event_outbox(created_at,id) where status in ('pending','leased');
create index if not exists event_outbox_tenant_pending_idx on private.event_outbox(organization_id,status) where status<>'completed';
create index if not exists event_outbox_health_idx on private.event_outbox(created_at) where status<>'completed';
create index if not exists event_outbox_worker_idx on private.event_outbox(worker_token_id);
create table if not exists private.access_decision_events (
 id uuid primary key default gen_random_uuid(),
 organization_id uuid not null references public.organizations(id),
...
create index if not exists access_decision_events_job_idx on private.access_decision_events(organization_id,processing_job_id);
...
create or replace trigger domain_events_immutable before update or delete on private.domain_events for each row execute function private.deny_domain_audit_mutation_v1();
create or replace trigger access_decision_events_immutable before update or delete on private.access_decision_events for each row execute function private.deny_domain_audit_mutation_v1();

-- Not callable by a user or the worker: only the protected mutation trigger produces events.
create or replace function private.append_domain_event_v1(p_id uuid,p_organization_id uuid,p_kind text,p_aggregate_id uuid,p_reason text,p_state jsonb,p_human_intervention_id uuid default null,p_retrieval_event_id bigint default null,p_correlation_id uuid default null)
...
create or replace function private.capture_authority_domain_event_v1() returns trigger language plpgsql security definer set search_path='' as $$
...
create or replace trigger membership_domain_event after insert or update or delete on public.organization_memberships for each row execute function private.capture_authority_domain_event_v1('membership');
create or replace trigger resource_grant_domain_event after insert or update or delete on private.resource_access_grants for each row execute function private.capture_authority_domain_event_v1('resource_grant');
create or replace trigger workspace_capability_domain_event after insert or update or delete on private.workspace_capability_grants for each row execute function private.capture_authority_domain_event_v1('workspace_capability');
create or replace trigger commercial_account_link_domain_event after insert or update or delete on private.account_organizations for each row execute function private.capture_authority_domain_event_v1('commercial_account_link');

create or replace function private.claim_event_outbox_v1(p_worker_token text)
...
create or replace function public.claim_event_outbox_v1(p_worker_token text) returns jsonb language sql security invoker set search_path='' as $$ select private.claim_event_outbox_v1(p_worker_token) $$;
...
create or replace function public.complete_event_outbox_v1(p_worker_token text,p_outbox_id uuid,p_capability text) returns jsonb language sql security invoker set search_path='' as $$ select private.complete_event_outbox_v1(p_worker_token,p_outbox_id,p_capability) $$;
...
create or replace function private.require_domain_event_propagation_v1(p_organization_id uuid) returns void language plpgsql security definer set search_path='' as $$
begin
...
create or replace trigger event_outbox_updated_at before update on private.event_outbox for each row execute function private.set_updated_at();
...
do $$ declare signature text; body text; marker text; replacement text; begin
 foreach signature in array array['private.authorize_pack_distribution(uuid,uuid,uuid,text,text,jsonb)','private.authorize_qualified_introduction_plan(uuid,text)'] loop
  -- Pack distribution is a recorded staging-only surface; do not promote it here.
  if signature like '%authorize_pack_distribution%' and to_regprocedure(signature) is null then continue; end if;
  body:=pg_get_functiondef(signature::regprocedure);
  if strpos(body,'perform private.require_domain_event_propagation_v1(')>0 then continue; end if;
...
create or replace function private.retry_blocked_event_outbox_v1(p_organization_id uuid,p_outbox_id uuid)
...
create index if not exists processing_jobs_live_outbox_authority_idx on public.processing_jobs(organization_id,id)
