-- integration_preview: the preview run carries a small model budget.
--
-- The skeleton's run spent nothing, so its job budget was one call and one cent. In live mode
-- the run makes one bounded call for the questions it asks from the gaps the objects declare
-- (and, in a later slice, one for the synthesis). The activation now gives the run three calls
-- and ten cents; the worker's per-job caps still bound it. Nothing else in the activation changes.
...
begin
  ...
  insert into public.processing_runs (
    ...
    v_pipeline_version,
    jsonb_build_object('maxCalls', 3, 'maxCostUsd', 0.10),
    jsonb_build_object(
      'planId', plan_row.id,
      'briefId', brief_id,
  ...
  insert into public.processing_jobs (
    ...
    jsonb_build_object(
      ...
      'model_budget', jsonb_build_object('max_cost_usd', 0.10, 'max_calls', 3),
