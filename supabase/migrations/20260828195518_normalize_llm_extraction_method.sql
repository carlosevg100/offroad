-- The extraction schema calls an anchored model-derived fact `llm_anchored`. A deployed
-- worker version still emitted the retired internal label `model_extraction`, which made an
-- otherwise successful read fail at the final insert. Normalize that legacy value at the
-- trusted command boundary so in-flight workers remain compatible, while new workers emit the
...
comment on function private.worker_record_candidates(uuid, text, jsonb) is
  'Capability-bound worker write. Normalizes the retired model_extraction label to llm_anchored before persistence.';
