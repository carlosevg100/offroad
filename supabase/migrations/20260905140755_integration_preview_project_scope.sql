-- integration_preview: the grant is scoped to projects, not to a whole organization.
--
-- An organization-wide grant diverted every turn of the founder's organization, old projects
-- included, to the Case 01 preview router. A grant now carries a scope: 'organization' keeps the
-- previous meaning (the local E2E stack relies on it), 'projects' limits the mode to the projects
-- listed in private.integration_preview_projects. The claim and the activation read the scope for
-- the job's project; the loaders and the completion keep asking the organization-level grant,
-- which stays true while any scope is enabled, because they only ever run for jobs the claim and
-- the activation already admitted. The status read by the banner returns the scope and the
-- listed projects so the screen marks only what runs in preview.

alter table private.integration_preview_grants
  add column if not exists scope text not null default 'organization'
  check (scope in ('organization', 'projects'));

comment on column private.integration_preview_grants.scope is
  'organization: every project of the organization runs in preview. projects: only the projects listed in private.integration_preview_projects.';

create table if not exists private.integration_preview_projects (
...
comment on table private.integration_preview_projects is
  'Projects that run the internal integration_preview mode when their organization''s grant has scope = projects. Written by operators, never through the Data API.';

revoke all on table private.integration_preview_projects from public, anon, authenticated;

create or replace function private.integration_preview_enabled_for_project(p_organization_id uuid, p_capital_project_id uuid)
returns boolean
language sql
security definer
set search_path = ''
  ...
  select exists (
    ...
    where grant_row.organization_id = p_organization_id
      ...
      and (
        ...
        or (
          p_capital_project_id is not null
          and exists (
            select 1
            from private.integration_preview_projects project_row
            where project_row.organization_id = p_organization_id
              and project_row.capital_project_id = p_capital_project_id
              and project_row.enabled
          )
        )
      )
  );
...
create or replace function private.integration_preview_enabled_for_session(p_organization_id uuid, p_intake_session_id uuid)
returns boolean
language sql
security definer
set search_path = ''
as $$
  select private.integration_preview_enabled_for_project(
    p_organization_id,
    (
...
revoke all on function private.integration_preview_enabled_for_project(uuid, uuid) from public, anon, authenticated;
revoke all on function private.integration_preview_enabled_for_session(uuid, uuid) from public, anon, authenticated;

-- The claim: the flag follows the job's project.
...
-- The activation: refused unless the job's project runs in preview.
...
-- The status the banner reads: enabled, scope and the listed projects of the caller's organization.
...
begin
  ...
  if not exists (
    ...
    return jsonb_build_object('enabled', false, 'scope', null, 'projectIds', '[]'::jsonb, 'note', null);
  end if;
