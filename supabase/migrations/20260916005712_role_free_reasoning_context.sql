-- Stage 1C: deploy the role-free worker before applying this migration.
-- Preserve private historical profiles; remove only model-context projections.

CREATE OR REPLACE FUNCTION private.worker_load_agent_context_before_execution_brief_v1(p_job_id uuid, p_capability_token text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION private.worker_load_capital_project_context_v4(p_job_id uuid, p_capability_token text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION private.worker_load_capital_project_context_v6(p_job_id uuid, p_capability_token text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  return jsonb_build_object(
    ...
    'prior_artifacts', coalesce((
      ...
      ) order by latest.task_id, latest.artifact_type)
      from (
        select distinct on (plan_task.task_id, artifact.artifact_type)
        ...
        order by plan_task.task_id, artifact.artifact_type, artifact.created_at desc, artifact.id desc
      ) latest
    ), '[]'::jsonb),
...
-- These compatibility helpers are internal only. Keep the stage 1B restriction explicit.
revoke all on function private.worker_load_agent_context_before_execution_brief_v1(uuid,text) from public, anon, authenticated;
revoke all on function private.worker_load_agent_context_before_professional_context_v1(uuid,text) from public, anon, authenticated;
