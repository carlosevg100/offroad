...
-- Recorded statements MD5 (joined with newline): a5a70e94d0ef6ac28ef6e55c027c6cc9
...
begin
  ...
  return jsonb_strip_nulls(jsonb_build_object(
    'scopeEventId', scope_event.event_id,
...
begin
  ...
  if session_row.journey <> 'originator' or session_row.advisor_authorization is null
    or session_row.advisor_authorization ->> 'status' = 'revoked'
    or p_event_id is null or char_length(trim(coalesce(p_reason, ''))) not between 3 and 1000 then
...
begin
  ...
  if session_row.journey <> 'originator' or session_row.advisor_authorization is null
    ...
    or jsonb_array_length(coalesce(session_row.advisor_authorization -> 'evidenceReferences', '[]'::jsonb)) = 0
    or p_event_id is null or char_length(trim(coalesce(p_reason, ''))) not between 3 and 1000 then
