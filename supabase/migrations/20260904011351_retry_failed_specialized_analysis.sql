-- A specialized advisor activation used to replay the latest job solely because an active brief
-- existed. When that job had failed, the UI said that work was activated but no new analysis was
-- queued. Retry the same governed DAG with a new immutable brief version and the user's updated
-- conversation context. Live or successful work remains idempotent.

create or replace function private.worker_retry_failed_advisor_specialized_job_v1(
  p_job_id uuid,
...
declare
  ...
  existing_run public.processing_runs;
  ...
  new_brief_id uuid := gen_random_uuid();
...
begin
  ...
  if job_row.kind <> 'agent_operation_brief'
    ...
    or analysis_scope not in ('company_debt_view', 'origination_thesis', 'capital_planning')
    ...
    raise exception 'invalid_advisor_specialized_retry' using errcode = '22023';
  end if;
  if analysis_scope = 'origination_thesis' and (
    char_length(trim(coalesce(brief_content ->> 'meetingContext', ''))) not between 10 and 5000
    or coalesce(char_length(brief_content ->> 'thesisToTest'), 0) > 3000
    or coalesce(char_length(brief_content ->> 'audience'), 0) > 240
    or (brief_content ? 'meetingDate' and brief_content ->> 'meetingDate' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$')
  ) then
    raise exception 'invalid_origination_thesis_retry' using errcode = '22023';
  elsif analysis_scope = 'company_debt_view' and (
    coalesce(char_length(brief_content ->> 'focus'), 0) > 3000
    or coalesce(char_length(brief_content ->> 'knownContext'), 0) > 5000
  ) then
    raise exception 'invalid_company_debt_view_retry' using errcode = '22023';
  elsif analysis_scope = 'capital_planning' and (
    char_length(trim(coalesce(brief_content ->> 'capitalIntent', ''))) not between 10 and 5000
    or coalesce(char_length(brief_content ->> 'knownConstraints'), 0) > 3000
    or coalesce(char_length(brief_content ->> 'decisionContext'), 0) > 3000
  ) then
    raise exception 'invalid_capital_planning_retry' using errcode = '22023';
  end if;
  ...
  if project_row.entry_job <> analysis_scope
    ...
    raise exception 'public_specialized_retry_not_allowed' using errcode = '42501';
  end if;
  ...
  if existing_job.status not in ('failed', 'poison', 'cancelled') then return null; end if;

  select run.* into existing_run
  from public.processing_runs run
  where run.organization_id = job_row.organization_id
    and run.id = existing_job.processing_run_id;
  if not found then raise exception 'capital_project_retry_run_not_found' using errcode = 'P0002'; end if;
  ...
  where brief.organization_id = job_row.organization_id and brief.id = existing_brief.id;

  insert into public.capital_project_briefs (
    id, organization_id, capital_project_id, request_id, brief_kind, brief_version,
    status, content, content_fingerprint, created_by
  ) values (
    new_brief_id, job_row.organization_id, project_row.id, source_message.id,
    analysis_scope, next_brief_version, 'active', brief_content, brief_fingerprint,
  ...
  insert into public.processing_runs (
    ...
    new_run_id, job_row.organization_id, session_row.id, next_run_no, 'manual', 'queued',
    existing_run.pipeline_version, existing_run.budget,
    coalesce(existing_run.versions, '{}'::jsonb) || jsonb_build_object(
      'briefId', new_brief_id,
      'retriedFromJobId', existing_job.id,
      'activatedBy', 'advisor_failed_retry_v1'
    ),
    source_message.created_by
  );
  ...
  insert into public.processing_jobs (
    ...
    new_job_id, job_row.organization_id, new_run_id, session_row.id,
    'capital_project_analysis',
    existing_job.payload || jsonb_build_object(
      'locale', source_message.locale,
      'capital_project_brief_id', new_brief_id,
      'trigger_event', jsonb_build_object(
        'type', 'advisor_semantic_route',
        'reason', 'failed_analysis_retry',
        'sourceMessageId', source_message.id,
        'assistantMessageId', assistant_message.id,
        'priorJobId', existing_job.id
      )
    ),
  ...
  set current_run_id = new_run_id,
      status = 'processing',
      processing_started_at = now(),
      processing_completed_at = null,
      pipeline_version = existing_run.pipeline_version,
      updated_at = now()
  ...
  return jsonb_build_object(
    ...
    'replayed', false,
    'retried', true,
    'retried_from_job_id', existing_job.id
  );
end;
$$;
...
begin
  ...
  if p_activation is not null then
    activated := private.worker_retry_failed_advisor_specialized_job_v1(
...
revoke all on function private.worker_retry_failed_advisor_specialized_job_v1(uuid, text, uuid, jsonb)
  from public, anon, authenticated;
revoke all on function private.worker_record_agent_response_and_activate_v2(uuid, text, uuid, jsonb, jsonb, jsonb)
  from public, anon, authenticated;
grant execute on function private.worker_record_agent_response_and_activate_v2(uuid, text, uuid, jsonb, jsonb, jsonb)
  to authenticated;

comment on function public.worker_record_agent_response_and_activate_v2(uuid, text, uuid, jsonb, jsonb, jsonb) is
  'Atomically records one advisor response and activates or safely retries the exact released public-information DAG with a new immutable brief version.';
