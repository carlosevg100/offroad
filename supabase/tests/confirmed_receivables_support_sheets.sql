...
values('20000000-0000-4000-8000-000000000901','40000000-0000-4000-8000-000000000901','50000000-0000-4000-8000-000000000901',1,'70000000-0000-4000-8000-000000000901','document_layer','2026.08.28-v1',repeat('a',64),repeat('b',64),encode(extensions.digest('xx'::bytea,'sha256'),'hex'),2,'xx');
...
update public.document_intake_sessions set result_summary=jsonb_set(result_summary,'{case_state,receivablesVertical,supportSheetCandidates}',
 '[{"documentId":"50000000-0000-4000-8000-000000000901","sheet":"Support"},{"documentId":"50000000-0000-4000-8000-000000000901","sheet":"Evidence"}]')
...
do $$ declare rejected boolean:=false; begin
 begin perform public.read_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901'); exception when insufficient_privilege then rejected:=true; end;
 if not rejected then raise exception 'cross tenant scope read allowed'; end if;
 rejected:=false;
 begin perform public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),'{"documentId":"50000000-0000-4000-8000-000000000901","sheet":"Titles","headerRow":1}','{}','2026-09-01','90000000-0000-4000-8000-000000000902',array['Support','Evidence']); exception when insufficient_privilege then rejected:=true; end;
...
-- Reject unknown/primary/duplicate support selections before any new run.
do $$ declare selected text[]; denied boolean; begin
 foreach selected slice 1 in array array[['Titles','Support'],['Missing','Support'],['Support','Support']] loop
  denied:=false;
  begin perform public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),'{"documentId":"50000000-0000-4000-8000-000000000901","sheet":"Titles","headerRow":1}','{}','2026-09-01',gen_random_uuid(),selected);
  exception when invalid_parameter_value then denied:=true; end;
  if not denied then raise exception 'invalid support selection accepted: %',selected; end if;
 end loop;
end; $$;
do $$ declare ctx jsonb; a jsonb; b jsonb; primary_tape jsonb:='{"documentId":"50000000-0000-4000-8000-000000000901","sheet":"Titles","headerRow":1}'; rejected boolean; begin
 ctx:=public.read_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901');
 if ctx->>'state'<>'unconfirmed' then raise exception 'expected unconfirmed'; end if;
 rejected:=false;
 begin perform public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('e',64),primary_tape,'{}','2026-09-01','90000000-0000-4000-8000-000000000901',array['Support','Evidence']); exception when serialization_failure then rejected:=true; end;
 if not rejected then raise exception 'stale manifest accepted'; end if;
 rejected:=false;
 begin perform public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),jsonb_set(primary_tape,'{headerRow}','2'),'{}','2026-09-01','90000000-0000-4000-8000-000000000901',array['Support','Evidence']); exception when invalid_parameter_value then rejected:=true; end;
 if not rejected then raise exception 'undiscovered candidate accepted'; end if;
 rejected:=false;
 begin perform public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),primary_tape,array['50000000-0000-4000-8000-000000000902'::uuid],'2026-09-01','90000000-0000-4000-8000-000000000901',array['Support','Evidence']); exception when invalid_parameter_value then rejected:=true; end;
 if not rejected then raise exception 'another pool accepted as complement'; end if;
 a:=public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),primary_tape,'{}','2026-09-01','90000000-0000-4000-8000-000000000901',array['Support','Evidence']);
 b:=public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),primary_tape,'{}','2026-09-01','90000000-0000-4000-8000-000000000901',array['Evidence','Support']);
 if a#>'{scope,primarySupportSheets}' is distinct from '["Evidence","Support"]'::jsonb then raise exception 'support selection not canonical'; end if;
 ...
 if public.read_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901')->>'state'<>'current' then raise exception 'confirmed scope not current'; end if;
 rejected:=false;
 begin perform public.confirm_receivables_evidence_scope_v2('40000000-0000-4000-8000-000000000901',repeat('d',64),primary_tape,'{}','2026-08-01','90000000-0000-4000-8000-000000000901',array['Support','Evidence']); exception when unique_violation then rejected:=true; end;
 if not rejected then raise exception 'replayed command accepted changed date'; end if;
 rejected:=false;
 begin perform public.worker_load_case_input_v3('80000000-0000-4000-8000-000000000901',repeat('x',64)); exception when insufficient_privilege then rejected:=true; end;
...
do $$ declare planner uuid; ctx jsonb; rejected boolean; old_name text; begin
 ...
 ctx:=public.worker_load_execution_brief_proposal_v4(planner,repeat('c',64));
 if ctx#>>'{confirmed_receivables_scope,state}'<>'current' then raise exception 'new proposal reader omitted scope'; end if;
 foreach old_name in array array['public.worker_load_execution_brief_proposal_v1','public.worker_load_execution_brief_proposal_v2','public.worker_load_execution_brief_proposal_v3','private.worker_load_execution_brief_proposal_v1','private.worker_load_execution_brief_proposal_v2','private.worker_load_execution_brief_proposal_v3'] loop
  rejected:=false;
...
do $$ declare ctx jsonb; p jsonb:='{"documentId":"50000000-0000-4000-8000-000000000901","sheet":"Titles"}'; begin
 ctx:=jsonb_build_object('candidates','[]'::jsonb,'supportSheetCandidates',jsonb_build_array(p||'{"sheet":"Support"}'::jsonb,p||'{"sheet":"Support"}'::jsonb));
 if private.receivables_support_sheets_valid(ctx,p,'["Support"]') then raise exception 'ambiguous sheet accepted'; end if;
 ctx:=jsonb_build_object('candidates',jsonb_build_array(p||'{"sheet":"Support","headerRow":3}'::jsonb),'supportSheetCandidates',jsonb_build_array(p||'{"sheet":"Support"}'::jsonb));
 if private.receivables_support_sheets_valid(ctx,p,'["Support"]') then raise exception 'other tape sheet accepted'; end if;
end; $$;
do $$ begin
 if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='worker_runtime_schema_contract_v1' and p.prosecdef) then raise exception 'public runtime contract must be invoker'; end if;
end; $$;
set local role authenticated;
do $$ begin
 if not (public.worker_runtime_schema_contract_v1()->'capabilities' ? 'confirmed-receivables-support-sheets.v1') then raise exception 'support sheet runtime capability missing'; end if;
end; $$;
reset role;
rollback;
