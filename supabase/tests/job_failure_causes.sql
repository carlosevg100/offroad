...
begin
  ...
  if private.job_failure_class('{"code":"quality_gate_m07_failed","stage":"test","retryable":false,"cause":{"name":"Error","class":"worker_error","message":"synthetic failure raised by the contract test"}}'::jsonb) <> 'quality_gate' then raise exception 'quality gate not classified'; end if;
  ...
  if not private.job_failure_has_cause('{"code":"budget_exceeded","stage":"test","retryable":false,"cause":{"name":"Error","class":"worker_error","message":"synthetic failure raised by the contract test"}}'::jsonb) then raise exception 'self-explaining gateway code not counted as a cause'; end if;
