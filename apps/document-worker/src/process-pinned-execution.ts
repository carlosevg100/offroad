 ...
 try {
  ...
  if(!exhausted){
   ...
   if(receipt.mayExecute){
    ...
    // An earlier lease settled this calculation with its exact bytes: publish them, never recompute.
    ...
    if(settled.available){
     ...
     resultText=text;reason="calculated";outcome="succeeded";
  ...
  if(reserved)await queue.settle(c,resultText);
  await renew();
  if(exhausted){reason="budget_exhausted";outcome="partial";resultText=partial(reason);}
