...
describe("R01 expand and retract lifecycle", () => {
  it("advances from documents to premises to a traced specialist result without parallel truth paths", () => {
    const built = buildReceivablesRawUniverse({universeId: "pool-1", datasetHash, documents: [document]});
    ...
    expect(evidenceQuestions.requests).toEqual([]);
