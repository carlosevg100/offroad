...
const deps=(output:unknown)=>({gateway:{complete:vi.fn().mockResolvedValue({output: output && typeof output === "object" ? {...output, issues: (output as {issues?:Array<{sourceIds?:string[]}>}).issues?.map(issue=>({...issue,sourceIds:issue.sourceIds?.map(id=>id==="alpha"?"p1":id==="beta"?"p2":id)}))} : output})} as unknown as Pick<ModelGateway,"complete">});
