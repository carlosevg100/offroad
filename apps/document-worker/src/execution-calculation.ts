...
try {
 ...
 const m=require(d.file), input=m.capitalProcedurePacketV2InputSchema.safeParse(JSON.parse(d.input));
 if(!input.success){parentPort.postMessage({ok:false,reason:'invalid_input'});}
 else {
  const output=m.capitalProcedurePacketV2OutputSchema.parse(m.prepareCapitalProcedurePacketV2(input.data));
...
export async function calculatePinnedCapital(identity: {methodId:string;methodVersion:string;manifestHash:string}, inputText:string, signal:AbortSignal):Promise<CalculationResult> {
  ...
  const result=await awaitExecutionThread(()=>new Worker(capitalProgram,{eval:true,workerData:{file,hash:release.artifactHash,input:inputText},
    env:{},execArgv:[],resourceLimits:{maxOldGenerationSizeMb:256,stackSizeMb:4},stdout:true,stderr:true}),signal);
