...
export const job: CapitalProjectAnalysisJob = {
  claimed: true,
  job_id: ids.job,
...
export async function runCapitalPlanningFixture(profile: string | null, failFinalArtifact = false) {
 const requests: unknown[] = [];
    ...
    let failed = false;
    const queue = {
      writeStage: async (_job: unknown, stage: string, status: string, detail: unknown) => { stages.push({stage, status, detail}); },
      ...
      recordCapitalProjectArtifact: async (_job: unknown, input: {
        ...
        if (failFinalArtifact && taskId === "S11") throw new Error("synthetic_persistence_failure");
