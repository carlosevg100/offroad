...
const settledResultSchema=z.union([
 ...
 z.object({available:z.literal(true),resultText:z.string().max(8_388_608),resultHash:hash,outcome:z.enum(["succeeded","partial"]),
  reason:z.enum(["calculated","invalid_input","calculation_failed","budget_exhausted","operation_uncertain"]),settledByLease:z.uuid()}).strict(),
]);
...
export type ExecutionQueue={claim():Promise<ExecutionQueueClaim|null>;renew(c:ExecutionQueueClaim):Promise<ExecutionRenewal>;reserve(c:ExecutionQueueClaim):Promise<z.infer<typeof reservationSchema>>;
 ...
 settle(c:ExecutionQueueClaim,resultText:string,outcome:ExecutionOutcome,reason:ExecutionReason):Promise<void>;commit(c:ExecutionQueueClaim,inputHash:string,resultText:string,outcome:ExecutionOutcome,reason:ExecutionReason):Promise<void>};
...
export function createExecutionQueue(client:SupabaseClient,workerToken:string):ExecutionQueue {
 ...
 return {
  ...
  async settle(c,resultText,outcome,reason){z.object({settled:z.literal(true),replayed:z.boolean()}).strict().parse(await rpc("worker_settle_execution_v2",{...lease(c),p_result_text:resultText,p_outcome:outcome,p_reason:reason},Buffer.byteLength(resultText,"utf8")));},
