...
export type ExecutionQueue={claim():Promise<ExecutionQueueClaim|null>;renew(c:ExecutionQueueClaim):Promise<ExecutionRenewal>;reserve(c:ExecutionQueueClaim):Promise<z.infer<typeof reservationSchema>>;
 settle(c:ExecutionQueueClaim,resultHash:string):Promise<void>;commit(c:ExecutionQueueClaim,inputHash:string,resultText:string,outcome:ExecutionOutcome,reason:ExecutionReason):Promise<void>};
export function createExecutionQueue(client:SupabaseClient,workerToken:string):ExecutionQueue {
 const rpc=async(name:string,args:Record<string,unknown>)=>{const {data,error}=await client.rpc(name,args).abortSignal(AbortSignal.timeout(5000));if(error)throw new Error("execution_transport_failed");return data;};
 ...
 return {
  ...
  async settle(c,resultHash){z.object({settled:z.literal(true),replayed:z.boolean()}).strict().parse(await rpc("worker_settle_execution_v1",{...lease(c),p_result_hash:resultHash}));},
  async commit(c,inputHash,resultText,outcome,reason){const r=z.object({committed:z.literal(true),replayed:z.boolean(),outcome:z.enum(["succeeded","partial"])}).strict().parse(await rpc("worker_commit_execution_v1",{
   ...lease(c),p_contract_hash:c.contractFingerprint,p_input_hash:inputHash,p_result_text:resultText,p_outcome:outcome,p_reason:reason}));if(r.outcome!==outcome)throw new Error("execution_commit_mismatch");},
 };
}
