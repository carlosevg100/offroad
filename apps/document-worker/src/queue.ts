...
export type QueueClient = {
  ...
  /** Stores exact generated bytes through a single-use, capability-bound Storage grant. The
   * worker still has no service-role key and re-downloads the object before marking it stored. */
  storeCapitalProjectMaterial?(job: CapitalProjectAnalysisJob, input: {
    bytes: Uint8Array;
...
export function createQueueClient(
  ...
  return {
    ...
    async storeCapitalProjectMaterial(job, input) {
      const localHash = createHash("sha256").update(input.bytes).digest("hex");
      if (localHash !== input.contentSha256) throw new Error("capital project material bytes do not match contentSha256");
      const authorization = z.object({
        grant_id: z.uuid(),
        object_path: z.string().min(1),
        state: z.enum(["authorized", "stored"]),
        storage_etag: z.string().nullable(),
        replayed: z.boolean(),
      }).parse(await call("worker_authorize_capital_project_material_upload_v1", {
        p_job_id: job.job_id,
        p_capability_token: job.capability_token,
      ...
      let storageEtag = authorization.storage_etag;
      if (!authorization.replayed) {
        const upload = await supabase.storage.from("case-artifacts").upload(
          authorization.object_path,
          input.bytes,
          {contentType: input.mimeType, upsert: false},
        );
        if (upload.error) throw new Error(`capital project material upload failed: ${upload.error.message}`);
        storageEtag = upload.data?.id ?? input.contentSha256;
      }

      const downloaded = await supabase.storage.from("case-artifacts").download(authorization.object_path);
      if (downloaded.error || !downloaded.data) throw new Error(`capital project material verification download failed: ${downloaded.error?.message ?? "empty object"}`);
      ...
      if (storedBytes.byteLength !== input.bytes.byteLength || storedHash !== input.contentSha256) {
        throw new Error("capital project material changed during storage");
      }
      if (!storageEtag) throw new Error("capital project material has no immutable storage version");

      const completed = z.object({
        object_path: z.string().min(1),
        storage_etag: z.string().min(1),
        replayed: z.boolean(),
      }).parse(await call("worker_complete_capital_project_material_upload_v1", {
        p_job_id: job.job_id,
        p_capability_token: job.capability_token,
        p_grant_id: authorization.grant_id,
    ...
    async loadExecutionBriefProposal(job) {
      return call("worker_load_execution_brief_proposal_v2", {p_job_id: job.job_id, p_capability_token: job.capability_token});
    },
    ...
    async loadAgentContext(job) {
      return call("worker_load_agent_context_v3", {
        p_job_id: job.job_id,
        p_capability_token: job.capability_token,
      });
    },
