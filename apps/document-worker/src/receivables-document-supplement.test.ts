...
function changeCell(document: ReceivablesEvidenceDocument, sheetName: string, ref: string, value: string): ReceivablesEvidenceDocument {
  return {...document, layer: {...document.layer, sheets: document.layer.sheets?.map(sheet => sheet.name !== sheetName ? sheet
    : {...sheet, cells: sheet.cells.map(cell => cell.ref === ref ? {...cell, v: value} : cell)})}};
}
...
describe("document supplement integrity", () => {
  ...
  it("includes governed fields and source dataset in idempotency identity", () => {
    ...
    const policy = document.layer.sheets!.find(sheet => sheet.name === "POLITICA")!;
    const key = policy.cells.find(cell => cell.v === "MAX_ATRASO_DIAS")!;
    const changed = changeCell(document, "POLITICA", key.ref.replace(/^[A-Z]+/, "B"), "45");
    const after = buildReceivablesDocumentSupplementPatch({phaseOne, documents: [changed]});
    expect(before.patch?.sections).toEqual(after.patch?.sections);
    expect(before.patch?.evidence).toEqual(after.patch?.evidence);
    expect(before.patch?.fields).not.toEqual(after.patch?.fields);
    expect(before.patch?.patchId).not.toBe(after.patch?.patchId);
    const anotherDataset = buildReceivablesDocumentSupplementPatch({phaseOne: {...phaseOne, datasetHash: "c".repeat(64)}, documents: [document]});
    expect(before.patch?.patchId).not.toBe(anotherDataset.patch?.patchId);
  });
  it.each(["2026-02-30", "31/04/2026", "29.02.2025"])("does not normalize impossible cash date %s", value => {
    const document = changeCell(evidenceDocument(), "RECEBIMENTOS", "B2", value);
    const result = buildReceivablesDocumentSupplementPatch({phaseOne, documents: [document]});
    expect(result.patch?.sections.cashReceipts).toBeUndefined();
    expect(result.omittedSections).toContain("cashReceipts");
  });
  it.each(["1.234", "1,234", "12.3456", "12,3456", "1.23.456,78"])("does not guess or round monetary input %s", value => {
