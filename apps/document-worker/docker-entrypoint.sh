...
if [ "${REQUIRE_VIRUS_SCAN:-true}" != "false" ]; then
  ...
  if [ "$clamd_ready" != "true" ]; then
    log "clamd.unavailable" "refusing to start the worker without a scanner"
    exit 1
  fi

  # Attempt background refresh. Freshness is monitored separately; this command is not an
  # attestation that the installed definitions meet policy.
