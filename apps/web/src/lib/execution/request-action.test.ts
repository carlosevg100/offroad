...
describe("capital execution request action", () => {
  ...
  it.each([["execution_payload_provenance_denied", "42501", "provenance_denied"], ["execution_source_pin_denied", "42501", "provenance_denied"], ["execution_basis_pin_denied", "42501", "basis_denied"],
    ["execution_contract_denied", "42501", "stale"], ["execution_budget_expired", "22023", "stale"], ["execution_input_limit", "22023", "invalid"], ["network", undefined, "unavailable"]])("reports %s from the request step as %s", async (message, code, expected) => {
  ...
  it("follows the execution the database names for a repeated request id and never scans the list", async () => {
    mocks.rpc.mockImplementation(async (name: string) => name === "execution_contract_basis_v1" ? {data: basis(), error: null} : conflict(id(70)));
    expect(await requestCapitalExecution(input)).toEqual({ok: true, executionId: id(70), replayed: true});
    expect(mocks.rpc.mock.calls.map(([name]) => name)).toEqual(["execution_contract_basis_v1", "request_work_execution_v1"]);
    expect(mocks.revalidate).not.toHaveBeenCalled();
  });
  it.each(["", "not-a-uuid", undefined])("reports a conflict when the detail names no execution (%s)", async (details) => {
    mocks.rpc.mockImplementation(async (name: string) => name === "execution_contract_basis_v1" ? {data: basis(), error: null} : conflict(details as string));
  ...
  it("does not follow a detail on a unique violation that is not the request conflict", async () => {
    mocks.rpc.mockImplementation(async (name: string) => name === "execution_contract_basis_v1" ? {data: basis(), error: null} : {data: null, error: {message: "duplicate key value", code: "23505", details: id(70)}});
    expect(await requestCapitalExecution(input)).toEqual({ok: false, error: "conflict"});
  });
