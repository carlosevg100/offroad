...
export default async function ExecutionPage({params}: {params: Promise<{locale: string; projectId: string; executionId: string}>}) {
  ...
  return <main className="work-executions"><Link href={`/${locale}/app/projects/${projectId}/executions`}>{t("detail.list")}</Link><h1>{t("detail.heading")}</h1><p>{project.project_name}</p>
