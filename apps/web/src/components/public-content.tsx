...
import investorStyles from "./public-investor.module.css";
...
export async function PublicPageContent({locale, page}: {locale: AppLocale; page: Exclude<PublicPage,"home">}) {
  ...
  if (page === "security") return <><PageIntro title={c.security.title} intro={c.security.intro} locale={locale} c={c}/><section className={`${styles.section} ${styles.catalogSection}`}><div className={styles.securityGrid}>{(["one","two","three","four"] as const).map((key,index) => <article key={key}><span className={styles.index}>0{index+1}</span><h2>{c.security[`${key}Title`]}</h2><p>{c.security[`${key}Body`]}</p></article>)}</div><details className={styles.socNote}><summary>{c.security.socTitle}</summary><p>{c.security.socBody}</p></details><p className={styles.finePrint}>{c.security.boundary}</p></section><section className={styles.wash}><div className={styles.section}><div className={styles.sectionHeading}><h2>{c.security.reviewTitle}</h2><p>{c.security.reviewBody}</p></div><Link className={styles.button} href={publicPath(locale,"demo")}>{c.security.cta}</Link></div></section></>;
  ...
  if (page === "companies" || page === "advisors" || page === "investors") return <>
    ...
    {page === "investors" && <section id="mandate" className={`${styles.section} ${styles.demoLayout} ${investorStyles.intake}`} aria-labelledby="mandate-title"><div><h2 id="mandate-title">{c.investorContact.title}</h2><p className={styles.demoIntro}>{c.investorContact.intro}</p><div className={styles.demoAgenda}><h3>{c.investorContact.stepsTitle}</h3><ol>{(["stepOne","stepTwo","stepThree"] as const).map(key => <li key={key}>{c.investorContact[key]}</li>)}</ol></div></div><DemoContact locale={locale} copy={{...c.demo,...c.investorContact}} audiences={c.who} investor/></section>}
