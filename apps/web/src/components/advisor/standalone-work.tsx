...
export async function StandaloneWork({locale, project}: {
  ...
  const [{data: messages, error}, copy] = await Promise.all([
    supabase.from("agent_messages").select("id, role, content, status, error_code, created_at")
  ...
  return <AdvisorProject
    ...
    messages={(messages ?? []).map(message => ({id: message.id, role: message.role,
      content: message.content, status: message.status, errorCode: message.error_code, createdAt: message.created_at}))}
