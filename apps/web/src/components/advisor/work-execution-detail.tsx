...
export async function WorkExecutionDetail({locale, projectId, view}: {locale: string; projectId: string; view: WorkExecutionView}) {
  ...
  const known = (group: "reasons" | "gapCodes" | "requirements" | "kinds", key: string) => t.has(`${group}.${key}`) ? t(`${group}.${key}`) : key;
  ...
  return <>
    ...
    <section><h2>{t("detail.title")}</h2>
      <dl className="execution-facts">
        ...
        {view.job ? <><dt>{t("detail.job")}</dt><dd>{view.job.status} · {t("detail.attempts", {count: view.job.attempts})} · {date(view.job.updatedAt)}{view.job.lastErrorCode ? ` · ${t("detail.lastError")}: ${view.job.lastErrorCode}` : ""}</dd></> : null}
    ...
    {result && !result.withheld ? <section><h2>{t("detail.resultTitle")}</h2>
      ...
      {packet ? <>
        ...
        <h3>{t("detail.gaps")}</h3>
        {!packet.informationGaps.length ? <p>{t("detail.noGaps")}</p> : <ul>{packet.informationGaps.map((g, n) => <li key={n}><strong>{known("gapCodes", g.code)}</strong>{g.subjectId ? ` (${g.subjectId})` : ""}: {g.reason}</li>)}</ul>}
        {packet.contractualGaps.length ? <><h3>{t("detail.contractualGaps")}</h3><ul>{packet.contractualGaps.map((g, n) => <li key={n}>{g.subjectId}: {g.code}</li>)}</ul></> : null}
