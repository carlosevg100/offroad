...
test("two people preserve private branches, compare conflicts and lose revoked access", async ({page, browser}) => {
  ...
  try {
    ...
    await expect.poll(() => second.locator(".app-rail").evaluate(el => el.getBoundingClientRect().width)).toBe(252);
    await second.evaluate(() => window.scrollTo({top: 0, behavior: "instant"}));
    await test.info().attach("contribution-conflict-desktop", {body: await second.screenshot({fullPage: true, animations: "disabled"}), contentType: "image/png"});
    await second.setViewportSize({width: 390, height: 844});
    await expect(conflict).toBeVisible();
    await expect.poll(() => second.locator(".app-rail").evaluate(el => el.getBoundingClientRect().width)).toBe(58);
