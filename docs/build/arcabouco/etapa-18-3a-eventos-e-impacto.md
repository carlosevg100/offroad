...
Migração B de `work_dependencies_and_continuity`, arquivo `supabase/migrations/20260925180927_work_dependency_events.sql` (carimbo de produção; em staging a versão registrada é `20260925174235`). Testes `supabase/tests/work_continuity_dependencies.sql`, extensão de `supabase/tests/rls_non_interference.sql`, `scripts/ci/test-dependency-update-concurrency.py` (duas sessões reais, no job de banco) e os testes de unidade do contrato de evento e do vetor de paridade com `continuation.ts`. Nada é recomputado neste incremento: o agendamento das execuções candidatas é o incremento 3B. Nenhum resultado e nenhuma decisão são reescritos.
...
2. Aplicar a migração em staging, depois em produção, de uma vez. Ela troca constraints de `private.domain_events`, onde toda mutação auditada grava, e por isso define `lock_timeout` de 5 segundos, como a reconciliação da etapa 4: se uma transação longa segurar a tabela, a aplicação falha logo e pode ser repetida, em vez de enfileirar as gravações de eventos. Os patches dependem dos corpos de `append_domain_event_v1` e `complete_event_outbox_v1` publicados em `20260916102242_reconcile_domain_event_audit_outbox.sql`; se o ambiente divergir, a migração para com `domain_event_version_contract_changed`, `domain_event_effect_contract_changed` ou `event_outbox_dependency_contract_changed`.
...
Aplicada em 25/09/2026, depois do deploy do worker com o contrato da PR #783: staging `20260925174235` e produção `20260925180927`. O texto armazenado nos dois bancos é o do arquivo (md5 `6cc2bc79a874340111cc86a86800d91b`, conferido antes da aplicação com o mesmo texto e depois dela no registro de migrações); os corpos efetivos de `append_domain_event_v1` e `complete_event_outbox_v1` ficaram iguais ao snapshot (sha256 conferido nos dois bancos); advisor de segurança em zero nos dois; os índices novos aparecem como não usados no advisor de desempenho, como esperado em tabela vazia.

## Correção de 25/09/2026

O agregado do procedimento e o id do evento de release de plataforma eram md5 convertido em uuid, sem a versão e a variante da RFC 9562 que o contrato do worker exige; todo evento `method_release` seria bloqueado na outbox. A migração `20260925232939_rfc_uuid_domain_event_identifiers.sql` os troca por UUID versão 5 (`extensions.uuid_generate_v5` no namespace URL), com `private.platform_method_release_event_id_v1` para o id do evento. Nenhum evento `method_release` existia em nenhum ambiente.
