...
Migração `supabase/migrations/20260925204502_work_dependency_recompute.sql` (carimbo de produção; em staging a versão registrada é `20260925204055`). Testes: seções 13 a 30 de `supabase/tests/work_continuity_dependencies.sql`, a extensão de `supabase/tests/rls_non_interference.sql`, o teste de duas sessões `scripts/ci/test-dependency-recompute-concurrency.py` (dois workers disputando uma candidata, no job de banco), o ajuste de `supabase/tests/platform_operator_identity.sql` (novo chamador do núcleo com sujeito explícito) e, em `packages/work-plan`, o vetor de paridade da chave de recomputação e os testes do plano por linhagem. A composição compartilhada entre web e worker e o módulo `apps/document-worker/src/dependency-recompute.ts` vêm numa segunda PR, sobre esta, com os testes de `packages/execution-request` (igualdade de bytes com o caminho web), de `dependency-recompute.test.ts` e `runtime-schema.test.ts` no worker e a prova `scripts/ci/verify-dependency-recompute.mjs` no job de banco; até o worker novo ser implantado, nada chama as RPCs novas e as candidatas de custo zero ficam agendadas sem execução.
...
## Composição compartilhada e loop do worker (segunda PR)

- `packages/execution-request`, pacote novo: `contract.ts` e `gates.ts` saem de `apps/web/src/lib/execution` sem mudança de comportamento, e `composeCapitalExecutionRequest` reúne a sequência que a ação de pedido sempre seguiu: gates que não precisam do pacote (registro da companhia, método liberado, situações), fontes sem bytes verificados, pacote vinculado sobre a base, contrato que fixa exatamente a versão da base, gate de voz e recibo fechado. A ação web (`requestCapitalExecution`) e o worker enviam os três textos que ela devolve. A igualdade de bytes é provada em `compose.test.ts`, que fixa os digests SHA-256 do contrato, do snapshot e dos gates que o caminho web produzia em `7eb90d00`, antes do pacote existir, com a mesma base, o mesmo pedido, os mesmos ids e o mesmo relógio; os testes da ação e da tela web continuam os mesmos.
- `apps/document-worker/src/dependency-recompute.ts`: `createDependencyRecomputeQueue` chama as quatro RPCs fechadas com o token do worker e um prazo de transporte que cresce com o tamanho do texto; `runDependencyRecomputeOnce` reivindica uma candidata, confere o que a raiz pediu (sem pergunta, objetivos, data de referência ou situações, `origin_unavailable`), pede a base do solicitante original (uma recusa que o banco gravou volta como está), confere organização, trabalho, versão e propósito da base (senão `basis_unavailable`), compõe com o id do pedido igual ao da candidata e submete. Uma recusa de composição é gravada pelo seu código (`composition_invalid` como `composition_failed`). Falha de transporte sobe como erro: o lease expira e a próxima reivindicação retoma a candidata, até o limite de tentativas. O módulo não importa gateway de modelo; o teste confere o texto do módulo e a prova de CI confere o bundle.
- `apps/document-worker/src/main.ts`: um loop ao lado do loop de jobs, como o da outbox, com até dez candidatas por volta e espera de cinco segundos quando nada é agendável. O log leva só ids, estado e código (`recompute.finished`, `recompute.poll.failed`). As execuções criadas são jobs comuns de execução fixada, tomados pelo loop de jobs.
- `apps/document-worker/src/runtime-schema.ts`: a imagem exige a capacidade `dependency-recompute.v1`; contra um banco sem esta migração ela para no boot, antes de reivindicar qualquer coisa.
- `scripts/ci/verify-dependency-recompute.mjs`, no job de banco, logo depois da prova da avaliação governada: trabalho de capital sintético com a base governada da prova do consumidor fixado, sobre o período adotado sintético; o método de capital liberado com o perfil derivado do manifesto; a concessão do produtor. O dono, autenticado pela API local, pede a execução raiz pelo caminho web (base v2, composição compartilhada, `request_work_execution_v2`); uma revisão nova da base troca uma decisão, e o efeito de dependência planeja uma candidata de custo zero; a conta do worker, autenticada e vinculada ao token, roda o loop pelo módulo do worker e produz a candidata uma vez. A prova confere a execução para o solicitante original, com o id do pedido igual ao da candidata, fixando a revisão cabeça, o recibo de gates sem bloqueio com as situações da raiz, um job na fila, a linhagem até a raiz, o pedido `scheduled` e a ausência de gateway de modelo no bundle.
...
- Ainda não há alarme para `recompute.poll.failed` nem para candidata agendada sem execução por muito tempo; os alarmes da outbox (`apps/document-worker/monitoring/event-outbox-alarms.json`) não cobrem o loop novo; eles entram no fechamento da etapa.
...
4. Depois do deploy do worker, conferir no log eventos `recompute.finished` sem `recompute.poll.failed` e, no banco, que nenhuma candidata `scheduled` fica sem execução além de um ciclo do loop.

Aplicada em 25/09/2026: staging `20260925204055` e produção `20260925204502`, antes do deploy do worker da segunda PR (o worker em produção exige só as capacidades que conhece e ignora a nova). O texto armazenado nos dois bancos é o do arquivo (md5 `994946a10e94c7113b0c586c318e91f2`); os corpos de `apply_dependency_event_v1`, `merge_dependency_update_request_v1`, `worker_runtime_schema_contract_v1`, `execution_contract_basis_v1`, `request_work_execution_producer_v1` e `request_work_execution_producer_v2` são iguais ao snapshot nos dois bancos; advisor de segurança em zero nos dois.
