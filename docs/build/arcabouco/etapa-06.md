...
A conferência final encontrou uma incompatibilidade da projeção administrativa com a política da etapa 3: `read_workspace_access_v1` listava o tombstone deny como grant. A migração adicional `20260916230240_workspace_access_allow_projection.sql` (staging `20260916230213`) filtra somente allow na lista legada, sem remover a negação. Antes em staging: leitura negada, sujeito ainda listado; depois: leitura negada, sujeito ausente. `workspace_access_projection.sql` comprova a transição e a preservação do deny. Tipos idênticos, zero lints de segurança e catálogo sem nova superfície; total de 324 migrações de produção. O E2E exige atualização da lista e 404 imediato; a ação revalida a página após sucesso. Provas em `etapa-06-access-projection-eval.json`.
