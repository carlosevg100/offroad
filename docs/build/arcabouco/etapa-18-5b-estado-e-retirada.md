...
Depois que a candidata ganha a execução ou o resultado, o fato passa a ser o job deles. Todas as tabelas são legíveis por quem lê o trabalho, mas de um job a pessoa lê só as colunas de estado (`id`, tipo, situação, rodada de processamento, datas; a concessão por coluna de `20260824235937_controlled_production_rollout.sql`), nunca o `payload` nem a execução fixada. O leitor lê só essas colunas e, para os jobs de cálculo vivos, a rodada de processamento deles (`public.processing_runs`, legível), que distingue o recálculo institucional do grafo (`institutional-dependency-recompute-v1`) de um turno da conversa. O leitor valida os ids antes de montar o filtro e, se qualquer leitura falhar, lança erro em vez de supor atividade. `apps/web/src/lib/advisor/work-activity.ts` classifica as linhas e dá as decisões, todas funções puras:
...
- `workHasOpenWait`: há espera aberta, e o cabeçalho diz "Aguardando decisão". O job retido para aprovação também espera uma pessoa e nunca roda, mas o lugar dele é o cartão de aprovação do plano, que nomeia a versão exata a aprovar; o cabeçalho continua a mostrar essa aprovação como antes e não trata job retido como espera, porque um job retido que o plano exibido não possui não tem ação a oferecer (ver os limites).
- `institutionalCalculationRuns`: o cálculo de um resultado institucional roda enquanto a candidata de recálculo que o produz está agendada (ela só sai de `scheduled` quando o job termina) ou enquanto roda um turno de cálculo da conversa (o cálculo pedido pela pessoa é um turno, e a pessoa não pode ligar o job ao resultado sem o `payload`). Sem nenhum dos dois, nada calcula o resultado.
- `executionsAreRunning` e `jobKindRunning`: o que a lista de execuções, as telas especializadas e o caso precisam saber. O detalhe de uma execução usa a situação do próprio job que a leitura da execução (`read_work_execution_v2`) devolve, pela mesma regra (`jobStatusRuns`).
...
|---|---|---|
| Conversa do trabalho, com e sem intake (`AdvisorProject`, `page.tsx`, `standalone-work.tsx`) | atualizava por sessão, tarefa ou mensagem, por ação pendente e pela consulta do plano | atualiza por `workShouldRefresh`; ocupada por `conversationIsWorking`; cabeçalho "Aguardando decisão" quando há espera e nada roda |
| Resultado institucional | laço próprio enquanto `queued` | sem laço próprio (a página atualiza pelo leitor); "Calculando" só enquanto o cálculo roda (`institutionalCalculationRuns`); senão, lacuna com o próximo passo (revisar dados e premissas) |
...
| Detalhe da execução | sem atualização (botão manual mantido) | atualiza enquanto o job desta execução, lido pela própria leitura da execução, estiver `queued` ou `leased` |
...
- `institutional-model-result-activity.test.tsx`: "Calculando" só com o cálculo em andamento e lacuna sem ele.
...
- Achado da primeira execução da CI desta PR: na jornada de `integration_preview`, depois de ajustar o plano e aprovar a versão nova, um job continuou `awaiting_approval` sem pertencer ao plano exibido, e o cabeçalho, que então tratava job retido como espera, ficou quatro minutos em "Aguardando decisão" (na nova tentativa o job não ficou). Não há ação para esse job na tela. O cabeçalho passou a mostrar como espera só a espera aberta; o job retido continua fora da atualização e do indicador de trabalho. Se esse job deve ser cancelado quando a versão do plano muda é pergunta para quem cuida do plano de execução.
- O cálculo institucional pedido pela pessoa é um turno da conversa, e a pessoa não lê o `payload` que liga o job ao resultado: enquanto qualquer turno de cálculo roda, um resultado em fila aparece como "Calculando"; quando o turno acaba sem o resultado, aparece a lacuna. Ligar exatamente o job ao resultado pede uma leitura nova no banco, fora deste incremento.
