...
- Achado da primeira execução da CI desta PR: na jornada de `integration_preview`, depois de ajustar o plano e aprovar a versão nova, um job continuou `awaiting_approval` sem pertencer ao plano exibido, e o cabeçalho, que então tratava job retido como espera, ficou quatro minutos em "Aguardando decisão de uma pessoa" (na nova tentativa o job não ficou). Não há ação para esse job na tela. O cabeçalho passou a mostrar como espera só a espera aberta; o job retido continua fora da atualização e do indicador de trabalho. Se esse job deve ser cancelado quando a versão do plano muda é pergunta para quem cuida do plano de execução.
