...
`scripts/ci/verify-dependency-recompute.mjs`, no job Database, com o módulo de recomputação e o cliente RPC do próprio robô contra a pilha local:

1. o perfil que a prova registra para o método de capital publicado tem teto acima de zero (250000 microdólares e 3 chamadas); o gatilho de armazenamento que mantém todo perfil em zero fica desligado só para essa linha, no banco descartável;
...
Até este incremento, o mesmo script registrava um perfil de custo zero e provava que o laço produzia sem pessoa a candidata planejada. As duas situações não cabem no mesmo banco: o método de capital só pode ter um perfil executável (o produtor recusa dois com `execution_profile_ambiguous`, e perfis são imutáveis), a guarda de proveniência do capital aceita um único manifesto, a identidade do manifesto exige o mesmo hash no perfil e na release, e uma release é única por procedimento, versão e hash; não há como pedir uma segunda release de capital com custo. O script passou a registrar o perfil com custo. O que o laço faz depois que a candidata está agendada é o mesmo nos dois casos e continua provado aqui; o agendamento automático de orçamento zero continua provado pelo planejador em WCD:803 e WCD:833 e pela corrida de dois robôs em `scripts/ci/test-dependency-recompute-concurrency.py`.
...
- **O capital tem uma única release que pode ser pedida.** `private.execution_capital_payload_current_v1` (etapa 17) só aceita o manifesto publicado `2c023cf7b7ec7e35b7f59d363a9b287cb245d3196cd431fc0c2bf1fc937f8478` na versão `2026.09.21-v4`, e a identidade do manifesto (`private.validate_execution_storage_identity_v1`) exige esse mesmo hash na release fixada. As duas primeiras rodadas desta prova tentaram uma segunda release de capital com custo: com outro hash no perfil, a candidata autorizada foi recusada na submissão com `execution_payload_provenance_denied` e registrada como falha; com o hash publicado no perfil sobre uma release de outro hash, a submissão parou em `execution_manifest_identity_mismatch`. Consequência fora deste incremento: publicar a próxima versão do capital exige atualizar essa guarda na mesma migração, senão todo pedido do capital com o manifesto novo, humano ou de recomputação, é recusado com `execution_payload_provenance_denied`. A recusa de identidade do manifesto não aparece com perfis que passaram pela verificação de armazenamento. Não são defeitos deste incremento; ficam registrados para a publicação da próxima versão.
...
- `dependency_recompute_wait_across_restart: PASS (...)` e `dependency_recompute_worker_loop: PASS (...)` (VR).
