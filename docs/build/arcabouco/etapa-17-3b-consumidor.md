...
Os testes do worker exercitam deadline real com CPU ocupada, revogação no heartbeat, shutdown, settlement/commit separados, retries e transporte. O teste concorrente acrescenta disputa entre limpeza e heartbeat. A CI E2E cria uma base sintética pelos comandos reais de definição e hipótese, remapeia o pacote financeiro para essas identidades e exige que o worker implantado retorne bytes canônicos e hashes iguais ao executor publicado, com uma operação settled. A prova completa tem 173 hipóteses e 63 definições; a prova reduzida não a substitui.
