...
**Entrega incorporada em 21/09/2026.** Religar pelo transporte governado os nove scripts de `packages/evals/scripts` contidos na etapa 16: `run-gold-baseline.ts`, `measure-extraction.ts`, `probe-structured-output.ts`, `measure-classification.ts`, `continue-document-work-product-live.ts`, `run-document-work-product-live.ts`, `run-intent-router-gold.ts`, `run-executive-synthesis-live.ts` e `run-advisor-response-live.ts`. A substituição exige capability de avaliação autorizada, matriz por conta/modelo/recurso, direitos, prazo e revogação por tentativa, inclusive reparo/fallback, com limites de custo preservados e negativas sem envio. Só retirar a suspensão após testes no caminho inteiro; nenhuma flag pode liberar o transporte direto. O baseline histórico já omite `turns`, `documents` e `sources` na montagem da base: corrigir e comprovar o dry-run completo nesse incremento antes de religá-lo. Até lá a contenção preserva o ramo offline, sem certificar seu funcionamento preexistente.
