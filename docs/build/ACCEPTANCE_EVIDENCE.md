## Etapa 19, incremento 5: templates de apresentação com estrutura semântica versionada, 27/09/2026

Migração nova `presentation_template_versions` (arquivo `20260927130000_presentation_template_versions.sql`, carimbo provisório; o líder aplica em staging e produção e renomeia), sobre `main` `b1d56d62`, mais `case-export`, web e robô. O problema medido: `presentation_templates` era mutável e sem histórico, o cofre fixava a linha mutável por id e fingerprint de referência, a definição era só visual e o robô renderizava sempre com o template da casa. Agora cada gravação é uma versão imutável e numerada em `public.presentation_template_versions` (definição visual, estrutura semântica com seções ordenadas, campos com tipo em `text`, `number`, `table`, `chart` e `source_list`, obrigatoriedade e audiências, e o fingerprint sha256 das duas em SQL); `presentation_templates` aponta para a versão corrente e é retirada em vez de apagada (gravar de novo revive com a próxima versão); `set_presentation_template_v1` mantém os três argumentos e ganha `p_structure` com a estrutura da casa como padrão (as funções de três argumentos saem, fixadas por md5 do corpo efetivo, inclusive a linha de autorização injetada em `20260915204116`); o mesmo conteúdo replica e o conteúdo de uma versão anterior move o ponteiro de volta sem linha nova; `read_presentation_template_v1` devolve id, número, estrutura, fingerprint e histórico da versão, e a nova `read_presentation_template_version_v1` devolve uma versão exata sob a mesma regra de acesso; o cofre passa a fixar `presentation_template_version_id` e o fingerprint da versão (`submit_vault_entry_version_v1` e `vault_reference_allowed_v1` restatadas por inteiro e fixadas por md5; linhas legadas mantêm a regra antiga); a nova `worker_read_presentation_template_version_v1` e a política `brand_templates_worker_select` deixam o robô renderizar a prévia com a versão corrente do projeto (logotipo conferido por hash ou omitido e reportado; casa quando não há versão, dizendo por quê), com o id e o fingerprint da versão no manifesto e na auditoria; a estrutura da casa vive em `packages/case-export/src/presentation-structure.ts` e a migração embute o mesmo JSON, com teste que compara os dois; o renderizador aplica a estrutura (ordem, omissão de blocos não nomeados e lacunas nomeadas no slide e na auditoria para cada campo obrigatório sem conteúdo, nunca conteúdo inventado) e todo arquivo grava `OffroadTemplateVersionId` e o fingerprint da versão; a tela de identidade visual mostra a versão em uso, edita a estrutura (ordem, obrigatórios, audiências), grava como nova versão e lista as anteriores (número, data e autor) sem identificador; backfill de toda linha existente como versão 1 com a estrutura da casa (produção e staging têm zero linhas; a prova SQL ensaia numa linha sintética). Desvio declarado: unicidade `(organization_id, template_id, fingerprint)` em vez de `(organization_id, fingerprint)`, para que o registro da organização e o de um projeto com o mesmo conteúdo não colidam. Provas: `client_presentation_templates.sql` reescrita (imutabilidade, versão 2 e ponteiro, replay, ponteiro de volta, cinco recusas nomeadas da estrutura, retirada e revivência, cofre na versão exata, backfill sintético, robô com e sem lease, tenant estranho e anon), `rls_non_interference.sql` estendida, unitários em `case-export`, web e robô, jornada `institutional-setup.spec.ts` com versão 1, versão 2 e captura. Sem Docker nesta máquina: SQL, lint de esquema e jornada rodam no CI; os 22 objetos novos estão no inventário com catálogos vazios até a captura do líder, e o check de inventário falha em `migration_absent_from_production_journal` até a aplicação. Tipos da web escritos à mão até a regeneração. Desenho, funções mudadas, pinos e o que fica com o líder em `docs/build/arcabouco/etapa-19-5-templates-versionados.md`.

## Etapa 19, incremento 1: contrato de domínio do protocolo de artefato, sem migração, 26/09/2026

Incremento só de contrato (PR #806), sobre `main` `b1d56d62`, sem migração, sem worker, sem rota e sem leitura de banco. **Novo** `packages/domain-contracts/src/artifact-protocol.ts`, exportado do índice do pacote, com `artifact-protocol.test.ts` (41 cenários; o pacote fecha com 98 testes verdes). O contrato define os tipos de artefato (`answer`, `material`, `workbook`, `model_result`, `work_product`, `execution_result`, `presentation`, `document`), as audiências (`internal`, `advisor`, `external`), as origens de revisão (`worker`, `person`, `legacy`) e os tipos de bloco (`section`, `paragraph`, `table`, `chart`, `number`, `cell_region`); os esquemas estritos de artefato (organização, trabalho, tipo, assunto, origem legada, ponteiro de cabeça), de revisão imutável (número, revisão anterior, audiência, origem, manifesto, fingerprint do manifesto, sha256 e tamanho dos bytes, rótulo legado verbatim), de bloco ordenado com chave estável e afirmações `{claimId, kind, value, unit, period, supportIds}`, e de vínculo de dependência (versão de fonte com versão de direitos, execução, resultado institucional, release de método, slot de premissa e revisão de que deriva). O manifesto `artifact-manifest.2026.09.26-v1` tem exatamente os campos da decisão 3 do plano, toda parte opcional explícita e nula, chave desconhecida recusada, bytes armazenados ou renderizados de forma determinística, e coerência exigida: resultado de execução aponta a execução, resultado de modelo aponta o resultado institucional, e rótulo legado nunca carrega método, execução ou retrato de insumos.

As regras puras: substância mínima (`material` com afirmação, fonte, execução ou resultado institucional; `informational` só para `answer` de esclarecimento sem afirmação e sem número, com anos e datas contando como período; senão `without_substance`, o código que o comando SQL vai recusar); mudança material, cosmética ou idêntica, com os motivos (afirmação, valor, versão de fonte, release de método, execução, resultado institucional, audiência) que a etapa 20 usa para exigir ato novo; derivação (os vínculos vêm do manifesto, nunca do texto do bloco; derivado de A e B exige as duas; ancestral não resolvido é reportado, nunca omitido) e restrição mais restritiva num único reticulado (`denied`, `internal`, `advisor`, `external`); frescor (`current`, `stale`, `unknown`, com `stale` prevalecendo sobre `unknown` e grafo incompleto nunca `current`); avaliação de release (`internal`, `released`, `blocked`) pelas quatro aprovações que existem hoje sobre a versão exata (decisão `confirm` com o mesmo fingerprint, revisão de pacote aprovada com o fingerprint do material, resultado institucional estabelecido, execução com recibo), com acesso de leitura ao trabalho exigido para toda audiência, `external` servida só `released`, e uma única função sem parâmetro de propósito para prévia e download; resolução da revisão exata (id pedido ou ponteiro de cabeça, nunca a mais nova por data); rotulagem de legado das quatro formas históricas (`capital_project_artifacts`, `case_artifact_manifests`, `institutional_model_results` concluídos, `deal_state_objects` de tipo `material_artifact`) com o fingerprint original verbatim, audiência `internal`, evidência só do que a linha carrega e recusa de fabricar método, execução, retrato de insumos ou vínculo de fonte; e cinco adaptadores para o manifesto v1 (contrato de decisão, manifesto de material renderizado, artefato de planilha institucional, produto de trabalho documental e pacote do procedimento de capital v2). Os quatro adaptadores de contratos externos usam tipos estruturais com fixtures copiadas das formas reais, porque `case-understanding` e `financial-model` dependem de `reconciliation`, que depende de `domain-contracts`; só `documentWorkProductSchema` é importado direto. O manifesto gerado de runtime dos métodos (`packages/credit-playbook/src/method-runtime-manifest.generated.ts`) foi regenerado pelo gerador do pacote, porque ele fixa por hash todos os fontes de `domain-contracts` e o módulo novo passa a fazer parte desse pin; as verificações de imutabilidade dos métodos publicados passam no arquivo regenerado.

O que este incremento não faz: não cria tabela, comando, gatilho nem backfill (incremento 2); não tem leitor autorizado nem toca as cinco rotas (incremento 3); não grava revisão de produtor algum (incremento 4); não calcula fingerprint (o de manifesto e o de bloco nascem em SQL na escrita e o contrato só os carrega). Questões abertas para o incremento SQL: a versão de direitos pode vir nula do produtor (o pacote v2 só traz ids de versão de fonte) e o vínculo em `artifact_dependency_links` precisa resolvê-la ou recusar; o backfill rotula revisões legadas sem julgar a substância (uma linha histórica sem afirmação, fonte ou cálculo projeta `without_substance`, e o bloqueio do comando vale para escrita nova); o fingerprint de resultado da execução vem do recibo, com a canonização própria dele, nunca do fingerprint JSON do pacote; e os dezoito tipos de `capital_project_artifacts` projetam como `work_product` até que o inventário do backfill diga o contrário.

## OK do fundador e abertura da etapa 19: protocolo único de artefato, blocos e derivação, 26/09/2026

O fundador deu o OK expresso da etapa 19 em 26/09/2026, depois do fechamento da etapa 18 e de uma explicação detalhada do que o protocolo faz e não faz; o registro verbatim está em `docs/build/arcabouco-stage0/FOUNDER-ACTS.md`. O plano de execução (`docs/build/arcabouco/etapa-19-execucao.md`) registra o ponto de partida medido em `main` e em produção, as decisões de desenho, os incrementos e a ordem de publicação, o pronto da etapa e os riscos com dono. Em produção, na abertura: 129 artefatos de projeto (todos rascunho, exceto sete briefs de reunião pendentes de confirmação e um substituído), 4 manifestos de materiais, nenhuma planilha versionada, nenhum template de apresentação, nenhum resultado do modelo institucional e nenhuma execução; o backfill é pequeno e todo artefato existente entra como legado com a evidência disponível, sem vínculo fabricado. A preparação das etapas 20 e 21 (notas de desenho e contratos, sem migração) pode correr em paralelo; a migração de cada uma só entra com o OK seguinte.
## Correção de web: a lista de execuções não mostra identificador interno, e o rodapé do painel de resultados abre a configuração, 26/09/2026

Dois achados do 5D registrados no fechamento da etapa 18 (`docs/build/arcabouco/etapa-18-6-fechamento.md`, limites). A lista de execuções registradas (`work-execution-list.tsx`, etapa 17) mostrava o identificador interno de cada execução ao lado do link; agora a execução é nomeada pela data do pedido e pelo estado, e o identificador fica só no endereço do link. O rodapé "Revisar dados e premissas" do painel de resultados do modelo usava um link de fragmento comum, que o roteador pode desfazer na atualização seguinte; agora usa a navegação por seção do 5D (`followWorkSectionLink`), a mesma do link para a atualização que espera adoção. Provas: a jornada `capital-execution-request.spec.ts` confere que a lista não mostra o identificador nem bloco de código e que o link aponta para a execução; a jornada `institutional-setup.spec.ts` abre a configuração pelo rodapé e confere o endereço e a seção ativa na navegação. Só web: sem migração, sem mudança no robô e sem texto novo nos catálogos. O detalhe da execução continua, por desenho da etapa 17, uma tela de evidência com impressões digitais e códigos; apresentar essa proveniência na linguagem de quem decide é item de desenho de produto para a próxima onda.

## Etapa 18 / 6: fechamento

A etapa 18 fecha no commit de código `ffe98ae3`: projeção de dependências e marcos, eventos e fatos de invalidação, recomputação delimitada pelo robô, continuação com base explícita, adoção, autorização e recusa, resultados do modelo institucional no grafo comum, atividade só por fatos gravados, integração e acabamento da web, saúde da recomputação com alarmes e as provas do pronto. Os catorze critérios do pronto têm prova direta no CI. No fechamento, a revisão da rota da companhia achou e corrigiu quatro defeitos anteriores à etapa que a paravam depois da primeira análise (#797 e #798, migrações aplicadas em staging e produção). A Quality rodou onze vezes no commit final, dez por disparo manual e uma pelo push, todas verdes na primeira tentativa e sem teste instável; journals e catálogos de produção e staging idênticos aos commitados; web e worker no commit final, com o boot do robô lido no CloudWatch; os oito alarmes em `OK` com ações; produção sem fixtures. Ficam com o fundador: o OK da próxima onda (etapa 19 em diante), o marco 3, a revisão dos 75 parâmetros de referência, a decisão sobre o COAF, os destinatários dos alarmes e a leitura do CloudWatch pelo papel de implantação, e os itens da etapa 17 ainda abertos. Nada da etapa 19 foi iniciado. [Fechamento](arcabouco/etapa-18-6-fechamento.md).

## Etapa 18, incremento 5D (web): o painel aponta para o recálculo que espera adoção, o pedido de execução parte da continuação e a recusa ocupada tem o seu aviso, 26/09/2026

Incremento só de web, sem migração e sem mudança no worker, sobre `main` com o 5C (`2d690f9b`); os tipos da web não mudam. Fecha os três limites que a nota do 5C deixou na web. O painel de resultados do modelo financeiro deixa de pedir um novo cálculo quando o recálculo já está pronto numa atualização aberta: lido da mesma leitura das atualizações (`work_update_view_v1`, agora lida uma vez e antes do resultado), sem leitura nova de banco, o painel diz que o resultado recalculado com os insumos atuais aguarda a decisão da pessoa em Atualizações (ou, com a atualização ainda aberta por outra parte, que ela aguarda outra etapa) e o link "Abrir a atualização" abre a seção Atualizações naquela atualização, trazida para a vista, focada e destacada. A regra: candidata institucional liquidada, resultado concluído e não atual, numa atualização aberta, cobrindo o resultado que o painel mostra. Cada continuação que espera uma execução (aberta, ou agendada com a execução terminada sem resultado) ganha, no seu cartão em Atualizações, o pedido de execução com o texto da continuação como primeiro objetivo, numa linha e com os espaços que o banco colapsa, e a base mostrada para referência; o endereço leva só o id, a tela lê a continuação sob a autoridade de leitura do trabalho, tudo continua editável e a ação valida o pedido como antes (só passou a ler o limite de 2000 caracteres de uma constante compartilhada); o banco liga a execução à continuação pelo objetivo. Quando o banco não aceita uma recusa com `work_update_job_busy`, a tela passa a dizer que um cálculo da atualização está terminando e que a pessoa pode tentar de novo em alguns segundos, com a confirmação aberta e o mesmo id de comando; os demais erros continuam como estavam. Testes unitários do modelo (recálculos que esperam adoção, preenchimento, objetivo), do componente, do painel, do formulário, da ação (`busy`) e do link de seção com alvo; jornadas `work-update-integration.spec.ts` (o painel com a atualização aberta e com a pronta, o link que abre a atualização e a continuação que abre o pedido preenchido e é agendada pelo banco) e `institutional-setup.spec.ts` (o painel aponta para a atualização que a jornada adota), com capturas. Desenho, limites e aplicação em `docs/build/arcabouco/etapa-18-5d-acabamento.md`.

## Correção: a rota da companhia continua depois da primeira análise incremental, 26/09/2026

Defeitos, medidos numa réplica local de todas as migrações: (1) toda sessão pertence a um projeto, então todo `case_analysis` incremental nasce retido em `awaiting_approval` com uma `execution_brief_proposal`; o replay e a recusa de `private.enqueue_incremental_deal_state_analysis` só olhavam `queued`, `leased` e `succeeded`, e uma segunda chamada com o job retido deixava dois jobs retidos e duas propostas, com a resposta dizendo `job_status: queued`; (2) `approve_advisor_execution_brief_v1` movia o caso de `confirmed` para `processing`, `worker_complete_job` o levava a `review_ready` e dali em diante todo gatilho era recusado com `confirmed_case_required`: a análise da estrutura, do plano de produção e do pacote nunca podia ser enfileirada; (3) achado da revisão do líder: `settle_stale_execution_jobs_v1` só encerra jobs `queued` vencidos e `leased` expirados, e um job retido cujo despacho deixa de ser atual (brief mais novo no projeto, edição pedida, documento ou pedido novo, plano substituído, projeto arquivado) ficava retido para sempre; com a recusa do item (1) isso bloquearia a rota.

Máquina de estados: pelo histórico (#120 e `20260821114132`; #318 e `20260829205848`; #331; #547 e `20260908035026`; #593 e `20260910230211`), a análise incremental só começa de um caso `confirmed` com oportunidade, e o caso continua `confirmed` enquanto a análise espera, roda, termina ou falha. Quem derivou foi a aprovação da #547; a precondição do enqueue e a conclusão do worker ficam como estão. Nota de desenho com a evidência: `docs/build/arcabouco/correcao-rota-da-companhia.md`.

Correção: a migração `20260926062533_deal_state_route`, aplicada em staging (`20260926062438`) e em produção (`20260926062533`) em 26/09/2026 pelo executor via MCP, com o texto gravado igual ao arquivo nos dois bancos (md5 `401eeb0e303a9af74e6c298305483a68`), os quatro corpos resultantes iguais ao snapshot nos dois bancos (sha256 de `pg_get_functiondef`: enqueue `6fe530d5...`, aprovação `692d68dd...`, liquidação `0aae7d22...`, predicado `1276ac00...`) e o advisor de segurança sem apontamentos, cria `private.execution_hold_is_live_v1` (o job retido ainda pode ser aprovado: despacho atual, proposta na fila ou turno da conversa em andamento na sessão; revogada de clientes), remenda por texto o enqueue e a aprovação com trechos únicos e exceções nomeadas (`deal_state_analysis_replay_contract_changed`, `execution_brief_approval_session_contract_changed`) e redefine `settle_stale_execution_jobs_v1` fixada pelo md5 do `prosrc` (`e8151062...`, exceção `stale_execution_settlement_contract_changed`). A mesma decisão repete o job retido vivo com o status real, outra decisão é recusada com `deal_state_analysis_awaiting_approval` enquanto ele vive, e um job retido morto não é repetido nem bloqueia; a liquidação, que o worker roda a cada reivindicação, cancela o job retido morto com `execution_approval_superseded` e o seu run, com as mesmas travas. A aprovação só move para `processing` a sessão que não está `confirmed`. Os corpos de partida da réplica têm os hashes medidos em staging e produção antes da aplicação. Em produção, o único job retido (um `capital_project_analysis` com plano corrente) continua vivo e não foi tocado pela liquidação. Nenhuma tabela, grant ou gate muda. O snapshot dos corpos efetivos foi regravado da réplica: enqueue alterado; aprovação, liquidação e predicado incluídos pela primeira vez; os outros 89 corpos iguais. Na web, com a análise retida a lacuna da #795 mostra a aprovação do plano como próximo passo, com o link para o cartão do plano no projeto (na oportunidade, para o projeto), e não oferece "Retomar a análise"; a retomada, os formulários de decisão e os avisos mostram a espera pela aprovação; textos nos dois catálogos.

Provas: `supabase/tests/deal_state_route.sql` percorre a rota inteira com os comandos reais (confirmação do diagnóstico, enfileiramento, aprovação, lease e conclusão pelo worker, decisões da estrutura, do plano e do pacote) com um job e uma proposta por decisão, replay com o job retido e em fila, recusa de outra decisão, sessão `confirmed` em todos os passos, caso arquivado sem análise assumida e acesso revogado recusado. Na etapa da estrutura prova também os retidos vivos (proposta na fila, despacho atual, turno em andamento depois de uma edição pedida: a liquidação não os toca e outra decisão é recusada) e os mortos (edição pedida sem turno em andamento; brief mais novo de uma `capital_project_analysis` no projeto): a mesma decisão enfileira um job novo retido com a sua proposta, outra decisão não é recusada, e uma passada da liquidação cancela o job morto e o seu run e deixa a sessão `confirmed`, sem tocar a `capital_project_analysis` retida com despacho atual. Falha em `main` (com a #797) na primeira resposta. O contorno do teste da #797 virou a prova de que o caso continua confirmado. Testes unitários cobrem a lacuna retida e o resultado da fila; a jornada `document-first-intake` confere a análise do entendimento retida uma vez, com uma proposta, a sessão confirmada e a lacuna sem botão. A verificação do inventário da etapa zero falha até o líder aplicar e reconciliar.

Fora do escopo, confirmado nesta leitura: os escritores da introdução só são alcançáveis com service role, `release_authorization` nunca é gravado pelo produto e o status `introduced` do plano nunca é permitido.

## Correção: a aprovação do pacote de materiais enfileira a análise que faz a triagem dos financiadores, 26/09/2026

Defeito: as duas ações de aprovação do pacote (`approvePrivateProjectMaterialPackage` no projeto e `approveMaterialPackage` na oportunidade) gravam o `package_review` aprovado e chamam `enqueue_deal_state_analysis` com o gatilho `material_package_approved`, mas `private.enqueue_incremental_deal_state_analysis` só aceitava `understanding_confirmed`, `structure_changes_requested`, `structure_confirmed` e `production_plan_approved` e levantava `deal_state_analysis_trigger_invalid`. A aprovação ficava gravada, a pessoa via o erro de processamento e a análise que monta a triagem governada (`screenMandates` no worker, que roda quando o pacote está aprovado e o fluxo permite o matching) nunca era enfileirada. Nenhuma migração aceitou esse gatilho; as ações vieram com a #318 (30/08/2026) e a #346 (02/09/2026). Medição do líder em produção, 26/09/2026: produção nunca teve objeto `package_review`, `match_screen`, `material_artifact` ou `production_plan`, então ninguém foi afetado.

Correção: a migração `20260926053701_material_package_approved_trigger`, aplicada em staging (`20260926053634`) e em produção (`20260926053701`) em 26/09/2026 pelo executor via MCP, com o texto gravado igual ao arquivo nos dois bancos (md5 `ce40aa02f997ba04229eacf1b183a735`), o corpo resultante igual ao snapshot nos dois (sha256 `c51fab36...`) e o advisor de segurança sem apontamentos, remenda por texto o corpo atual da função. O gatilho passa a ser aceito, aponta para a última versão do `package_review` e exige que ela esteja `approved`, a mesma regra de `production_plan_approved`. São três trechos antigos, cada um presente exatamente uma vez; se não, ou se o gatilho já estiver no corpo, a migração para com `material_package_trigger_contract_changed` antes de mudar qualquer coisa. Assinatura, grants, modo de segurança, search_path, replay por fingerprint, recusa com análise em andamento, orçamento e forma do run e do job ficam iguais, e nenhuma tabela é tocada. O snapshot `docs/build/schema-history/effective-function-bodies` foi atualizado a partir de uma réplica local descartável de todas as migrações (Postgres 18), cujos 86 corpos anteriores são iguais ao snapshot; o diff do corpo são só as três inclusões. Na web, `analysis-gap.ts` ganha a lacuna `match_screen`: pacote aprovado sem a triagem governada do pacote e dos materiais atuais, sem análise em andamento, retomada com `material_package_approved` pela ação "Retomar a análise" da #795. No projeto, a área de mercado mostra essa lacuna no lugar do aviso "Ainda não há uma shortlist governada"; na oportunidade, a lacuna ocupa o estado "Pacote aprovado", que ficou inalcançável. As mensagens desses dois estados saíram dos catálogos, e as da lacuna entraram nos dois idiomas.

Provas: `supabase/tests/material_package_analysis_trigger.sql` (rollback) recusa o gatilho sem revisão do pacote e com revisão que pede ajustes (`current_deal_state_trigger_required`); com a revisão aprovada, enfileira um único `case_analysis` com `incremental_trigger = material_package_approved`, o fingerprint do pacote, o run com o mesmo gatilho e fingerprint e o teto de 3,10, retido em `awaiting_approval` com a sua proposta de brief de execução; depois da aprovação da execução pelo comando de produção e com o caso confirmado, a mesma chamada é replay (`deduplicated: true`, mesmo job); gatilho desconhecido e pessoa sem acesso continuam recusados. Em main o arquivo falha com `deal_state_analysis_trigger_invalid`. Testes unitários cobrem a lacuna presente, ausente com análise em andamento, ausente com a triagem existente, lida das linhas (só conta a triagem do pacote atual) e retomada com o gatilho do pacote. Nenhuma jornada Playwright aprova um pacote.

Achados fora do escopo, sem mudança aqui, medidos na réplica: (1) todo caso pertence a um projeto, então todo `case_analysis` incremental nasce retido em `awaiting_approval`, com uma `execution_brief_proposal`; o replay e a recusa por análise em andamento só olham `queued`, `leased` e `succeeded`, e uma segunda chamada com o job retido cria outro job retido e outra proposta (`deduplicated: false`), com a resposta dizendo `job_status: queued`; as lacunas da #795 aparecem nesse estado e oferecem "Retomar a análise", que repete a duplicação. (2) A aprovação da execução (`approve_advisor_execution_brief_v1`) move o caso de `confirmed` para `processing`, e o fim do job (`worker_complete_job`) o leva a `review_ready`; daí em diante a função recusa qualquer gatilho com `confirmed_case_required`. Como a análise do plano aprovado precisa rodar para o pacote existir, numa sessão real a aprovação do pacote deve cair nesse erro mesmo com esta correção.

## Etapa 18, incremento 5C (banco e web): adoção de todo dependente, recusa que interrompe, fim da continuação e uma ordem de travas para a aprovação, 26/09/2026

A migração `20260926051417_work_update_integration.sql`, aplicada em staging (`20260926051108`) e em produção (`20260926051417`) em 26/09/2026 pelo executor via MCP, com o texto gravado igual ao arquivo nos dois bancos (md5 `6c00fc0b4b63644b1e12203d4a71f743`), as travas das quatro redefinições e da visão conferidas antes nos dois bancos, os cinco corpos alterados por texto iguais ao snapshot por SHA-256 do `pg_get_functiondef` nos dois, o advisor de segurança sem apontamentos e zero marcos preenchidos em produção, fecha as costuras entre 3B, 4 e 5A. Todo resultado do modelo institucional concluído ganha o seu marco `execution_result` (sujeito `institutional_model_result`) pela visão única de mapeamento do incremento 2, com preenchimento dos existentes; `adopt_work_update_v1` adota numa só decisão as candidatas liquidadas de execução e as institucionais, e o marco de adoção referencia os resultados novos das duas espécies e os que eles substituem. Um recálculo institucional deixa de substituir o resultado anterior quando conclui: só a adoção o torna atual, e a leitura de resultados e o histórico mostram como atual apenas um resultado pedido por pessoa ou um recálculo adotado; o anterior fica guardado e apontando para o novo. `decline_work_update_v1` passa a recusar também candidatas agendadas das duas espécies e cancela os jobs do que recusa pelo caminho da varredura de autoridade (`last_error` `person_declined`), travando os jobs antes da linha do projeto e da trava da obra, a ordem dos caminhos do worker; nas duas ordens da corrida, quem toma o job primeiro vence e nada é gravado depois de uma recusa. Um `user_followup` passa a `scheduled` quando a próxima execução que uma pessoa pede na obra cita o seu texto nos objetivos (`private.work_followup_executions` guarda a execução e a base), a `ready` quando essa execução grava o resultado, a `adopted` quando a pessoa adota o resultado como nova base (o marco referencia o resultado e a base substituída) e a `declined` quando a pessoa recusa com motivo. O aprovar e calcular e as duas funções de revisão que ele chama travam a linha do projeto `for no key update`, e o passo do grafo que não enfileira o resultado da aprovação volta sem deixar efeito nem trava; duas sessões reais provam aprovação e planejador nas duas ordens sem 40P01 (em `main`, o planejador primeiro produz o impasse). Na seção Atualizações, uma atualização incorporada a uma mais antiga aparece dentro dela, o modelo financeiro aparece como dependente com fatos, bloqueios e recálculo, um recálculo agendado pode ser recusado sozinho e as continuações aparecem com base, execução, adoção e recusa. Testes: `supabase/tests/work_update_integration.sql`, extensão de `rls_non_interference.sql`, ajustes de `institutional_result_dependencies.sql` e `work_continuation_commands.sql`, `scripts/ci/test-work-update-integration-concurrency.py`, testes unitários do modelo e do componente, a jornada `work-update-integration.spec.ts` (adoção de uma atualização mista e recusa de um recálculo antes de o worker rodá-lo) e o ajuste de `institutional-setup.spec.ts`, que agora adota o recálculo. Nenhum objeto público novo; os tipos da web não mudam. Desenho, estados, ordem das travas, limites e aplicação em `docs/build/arcabouco/etapa-18-5c-integracao.md`.
## Etapa 18, incremento 6B (testes): as três provas que faltavam no pronto da etapa, 26/09/2026

O mapa de evidências dos catorze critérios do pronto da etapa 18 encontrou onze com prova direta no CI e três sem. Esta PR acrescenta as três, só com testes, sem migração e sem mudança de produto; os casos SQL terminam em rollback e os scripts gravam só no banco descartável do CI. Espera humana resiste a reinício: o script novo `scripts/ci/verify-dependency-wait-restart.mjs` roda por último no job Database, sobre o banco local refeito a partir das migrações (`supabase db reset --local`), porque registra o perfil do método de capital com teto acima de zero (gatilho de armazenamento desligado só para essa linha) e o capital admite um único perfil executável e uma única release que pode ser pedida; a nova revisão da base planeja a candidata da linhagem como espera de uma pessoa, marco `awaiting_human`, sem job e sem lease; o laço em execução chega a ocioso, um cliente novo com outra conta e outro token de robô (novo dono de lease) também, e a candidata e a espera continuam idênticas; o dono autoriza por `authorize_work_update_v1` e o laço reiniciado produz exatamente uma execução, para o solicitante original, com linhagem; a saúde do 6A conta a espera e depois a candidata agendada. `scripts/ci/verify-dependency-recompute.mjs`, a prova do 3B com o laço real e a candidata de orçamento zero produzida sem pessoa, continua exatamente como em main, com a leitura de saúde do 6A. Custo confirmado uma vez: a seção 8b de `work_continuation_commands.sql` autoriza a candidata com custo, deixa o lease da primeira reivindicação expirar e produz a candidata com a segunda; uma execução, o lease velho recusado, nenhuma espera nova, autorização e teto usados uma vez; a execução produzida tem uma conta de orçamento em zero, a reserva do teto autorizado é recusada (`execution_operation_denied`) e a reserva de custo zero da única operação se repete sem somar, também sob o lease de um job reivindicado de novo. Grafo incompleto: a seção 31 de `work_continuity_dependencies.sql` aposenta pelo comando do operador a única release de um procedimento fixado por uma execução; o evento seguinte do procedimento grava o fato `graph_incomplete` com a lacuna `head_unknown`, o planejador retém a execução com o sinal `method_release` do procedimento e nada é planejado nem reaproveitado, embora a fonte fixada não tenha mudado; a release seguinte libera a retenção e o próximo planejamento produz uma candidata. Limites registrados, sem mudança de produto nesta PR: reservas com custo são recusadas por `reserve_execution_operation_v1` e todo perfil gravado fica em zero, então nenhuma candidata de produção pode esperar autorização de custo hoje e o caminho de custo só existe com perfil sintético; a aposentadoria de uma release não emite evento, então o fato `graph_incomplete` aparece com o evento seguinte do procedimento, e o planejamento continua correto nesse intervalo; publicar a próxima versão do capital exige atualizar na mesma migração a guarda de proveniência, que só aceita o manifesto publicado. Tabela dos catorze critérios, com prova, job do CI e tipo, em `docs/build/arcabouco/etapa-18-6b-provas-do-pronto.md`.

## Etapa 18, incremento 5B (web): atividade pelos fatos gravados e retirada dos caminhos substituídos, 25/09/2026

As superfícies de trabalho e de execução passam a mostrar atividade só a partir de fatos gravados. Um leitor no servidor (`loadWorkActivity`, `apps/web/src/lib/advisor/work-activity-reader.ts`) lê, sob a autoridade de leitura do trabalho e só pelas colunas de estado que a pessoa pode ler de um job, os jobs vivos do trabalho e da sua sessão de intake (`queued` e `leased` são trabalho de máquina; `awaiting_approval` espera uma pessoa), as esperas abertas (`awaiting_human` sem `human_resolved` e sem espera mais nova; a de uma candidata de recálculo só enquanto ela aguarda autorização), os pedidos de atualização `scheduled` e as candidatas agendadas ainda sem execução ou resultado. A página só se atualiza enquanto há trabalho de máquina (`workShouldRefresh`) e para quando ele acaba; uma espera aberta aparece como "Aguardando decisão" e não é consultada, e o job retido para aprovação continua no cartão de aprovação do plano, sem atualização; a conversa fica ocupada só pelos jobs que ela espera (`conversationIsWorking`). Cada superfície lê a atividade antes dos resultados. Aplicado à conversa do trabalho com e sem intake, ao resultado institucional ("Calculando" só enquanto o cálculo roda), às telas especializadas de companhia e de planejamento de capital, à lista e ao detalhe de execuções e à página de oportunidade, que continua alcançável pelo trilho, pelos recentes e pela confirmação do intake. Um resultado ausente sem nada em andamento vira lacuna com o próximo passo: no caso privado e na oportunidade, "Retomar a análise", que decide a decisão pelas linhas gravadas e chama `enqueue_deal_state_analysis` (seguro de repetir); nas telas especializadas, a volta à conversa; no resultado institucional, a revisão de dados e premissas. Retirados: a tela dedicada de tese de originação, inalcançável desde a #364, com a sua inferência, o seu laço e o namespace `App.origination`; `advisorIsActive`, `advisorShouldRefresh`, `planPreparationStatus` e a consulta do job do plano; a atualização durante ação pendente; o laço próprio do resultado institucional; a consulta de jobs de `loadDealStateWorkbench`; e as cinco inferências pela ausência (oportunidade, materiais do caso privado, telas especializadas, tese de originação, texto da conversa vazia). O caminho de atualização por última mensagem que o incremento 4 substituiu já saiu na #789; o rascunho nomeado continua de propósito. Sem migração e sem mudança de worker. Testes: `work-activity.test.ts`, `analysis-gap.test.ts`, `institutional-model-result-activity.test.tsx`, `private-analysis-gap.test.tsx`, `advisor-project-state.test.ts` reescrito e a jornada Playwright `work-activity.spec.ts` (sem atualização depois do fim do job; espera mostrada como espera por pessoa). Nota: `docs/build/arcabouco/etapa-18-5b-estado-e-retirada.md`.

## Etapa 18, incremento 6A: saúde da recomputação e seus alarmes como configuração revisada, 26/09/2026

A migração `20260926024249_dependency_recompute_health.sql`, aplicada em staging (`20260926024214`) e em produção (`20260926024249`) em 26/09/2026 pelo executor via MCP, com o texto gravado igual ao arquivo nos dois bancos (md5 `d4cb7dffec4ed4738158017567b9ea90`), o corpo efetivo de `worker_runtime_schema_contract_v1` igual ao snapshot nos dois (sha256 `8be807b3...`, 23 capacidades), a leitura respondendo zeros em produção em 5,4 ms e o advisor de segurança sem apontamentos, cria o leitor `private.dependency_recompute_health_v1()`, fechado a todos os papéis de API, e a entrada do robô `public.worker_dependency_recompute_health_v1(p_worker_token)`, `security invoker` sobre um núcleo privado que exige o robô de recomputação como a reivindicação do 3B (`private.require_recompute_worker_v1`). A leitura devolve só quatro inteiros, somados sobre todas as organizações: `scheduledCount` e `oldestScheduledSeconds` sobre candidatas de execução e institucionais em `scheduled`, `expiredLeaseCount` (candidatas de execução sem a sua execução cujo lease venceu depois de uma tentativa) e `awaitingAuthorizationCount`, só informação. A idade vem de `updated_at`: nas máquinas de estado do 3B e do 5A uma candidata agendada só é gravada quando entra em `scheduled` e quando o que produziu é anexado, então `updated_at` marca a entrada na espera atual; `created_at` contaria a espera por pessoa do incremento 4 e dispararia o alarme ao autorizar uma candidata antiga, e o lease, em tabela própria, nunca zera a idade. A capacidade `dependency-recompute-health.v1` entra em `worker_runtime_schema_contract_v1` por patch de texto com agulha única e exceção `dependency_recompute_health_capability_contract_changed`, com o corpo efetivo atualizado no snapshot. Nenhum índice novo: numa réplica local com 500 000 candidatas de execução e 100 000 institucionais a leitura leva 3,3 ms pelos índices existentes (6,6 ms sem paralelismo). O loop do robô lê a saúde antes de cada reivindicação quando já passaram 30 s, registra `recompute.health` com os quatro números e `recompute.backlog.failed` acima de 1800 s ou com lease vencido; leitura com falha ou fora do contrato estrito registra `recompute.poll.failed` com `health_failed` e não para o loop; o robô exige a capacidade nova. Os alarmes da recomputação passam a configuração revisada em `apps/document-worker/monitoring/dependency-recompute-alarms.json` (heartbeat ausente por 3 min, atraso acima de 30 min por 2 min, lease vencido, e `offroad-recompute-errors` igual ao medido), e `scripts/ci/configure-event-outbox-alarms.py` recebe o arquivo como argumento, com o do outbox como padrão, amostra por filtro e descrição por arquivo, sem criar credenciais, sem tocar IAM, sem notificar e preservando ações. Testes: `dependency_recompute_health.sql` (estados de 3B e 5A, lease vencido, candidata institucional, recusas da ligação do robô, concessões), a extensão de `rls_non_interference.sql`, testes de unidade do robô (limite de 30 s, cada limiar, falha que não para o loop, parse estrito, campos que os filtros leem), testes do script para os dois arquivos e o passo de CI da recomputação, que agora exige uma linha `recompute.health`. `apps/web/src/types/database.ts` ganha a entrada da função pública. Ordem: migração, depois robô, depois o lead aplica o arquivo de alarmes com sessão AWS autorizada, liga o tópico `offroad-outbox-alerts` nos três alarmes novos e registra o resultado. Nada disso foi aplicado a banco ou conta AWS nesta PR. Nota de desenho: `docs/build/arcabouco/etapa-18-6a-saude-da-recomputacao.md`.
...
Os quatro arquivos de contexto seguem os identificadores de produção 20260908145938/41/44/47. Staging recebeu os mesmos corpos com carimbos anteriores do MCP; novas aplicações devem conferir nomes e corpos no histórico, sem repetir essas migrações por diferença de carimbo. A correção de predecessor foi aplicada após o gate de banco e está alinhada ao registro de produção `20260908155133`; advisor de segurança permaneceu sem alertas. A migração de capability continua local e não aplicada em produção.

Controles afetados: DATA-02/03/13, APP-02/03/04/09/11. Fluxo: candidatos e fontes privados do intake → leitor de capability → composição determinística → snapshots interno/visível privados. Sem novo provedor, envio externo, telemetria de conteúdo, backfill ou alteração de retenção. Testes negativos cobrem capability inválida, tenant incorreto, revisão ausente, fonte alterada/quarentenada, período inválido e divergência entre snapshots. A homologação destes testes não promove o programa enterprise inteiro.

Rollout: aplicar as quatro migrações aditivas de contexto e a correção de predecessor antes da ativação do worker; publicar e verificar a web compatível; somente então aplicar a migração que anuncia a capability `governed-sector-planning-context.v1`, permitindo iniciar o worker novo. O probe impede o novo worker de assumir trabalhos sem essa capability. Preservar RPCs e schemaVersion anteriores mantém o worker antigo compatível. Contenção: reverter o worker para a revisão anterior e manter migrações e leitor web compatível com planningContext; o novo fingerprint conserva a invalidação segura. Um rollback da web anterior ao novo contrato pode ocultar briefs novos por validação estrita e exige correção compatível, não remoção dos dados. Não reescrever snapshots ou restaurar aprovações antigas. Evidência final será registrada após os gates.
...
| Gate de alegação | `evaluateAssuranceClaim` + `security-assurance-statements.ts` | o gate legado permanece limitado a estados internos; metadata de attestation fornecida pelo caller nunca promove claim formal. SOC 2/ISO/pentest/auditoria usam o boundary assinado com bytes, trust root, escopo, validade e revogação |
...
| Evidência | Comando/artefato | Resultado | Data |
| --- | --- | --- | --- |
| Fonte canônica | `current-security-inventory.ts` | baseline do commit `b2e38975` cobre 8 claims canônicos por domínio, ambientes, classes de dados, sistemas, stores, fluxos, identidades, vendors, evidências, owners funcionais e 18 gaps obrigatórios; nenhum valor de segredo é armazenado | 2026-09-07 |
| Validador fail-closed | `security-current-state.ts` e `security-current-state.test.ts` | a avaliação síncrona é explicitamente declaration-only; o gate confiável verifica o remote autorizado e o commit real em `origin/main`, resolve os bytes do objeto Git e valida bytes/hash/metadados/autoridade allowlisted das observações. Recusa path, hash, collector, origem, ambiente, datas, claim, critério, gap, severidade ou status forjado, além de segredo antes do parse | 2026-09-07 |
...
| Claims externos governados | `security-assurance-statements.ts`, `ASSURANCE_CLAIM_TRUST_BOUNDARY.md` e testes | claim positivo exige scope fingerprint exato, evidência e bytes resolvidos, hash, assinatura Ed25519, `keyId`, tipo real `ed25519`, root do registro interno fingerprintado, emissor/claim autorizados, validade e ausência de revogação. Root e relógio fornecidos pelo caller são ignorados; decisão reconstruída falha fechada. Milestone concluído exige receipt que vincula catálogo, ID, framework, tipo, status, escopo, evidence ref e hash resolvido; uma evidência real de outro registro não pode completar um milestone arbitrário. O registro de roots atual é vazio e a vista permanece não certificada/não atestada | 2026-09-07 |
| Lint de prose de assurance | `security-assurance-language.ts` | defense in depth conservadora aplicada somente a campos narrativos não tipados. Saída canônica é autorizada pelo objeto/receipt do renderer; não existe allowlist textual do caller nem subtração de strings. Linguagem livre de alto risco e polaridade mista voltam para revisão; estados futuros e milestones legítimos usam a rota tipada | 2026-09-07 |
| Gap de rollout | observação de operador e histórico de rollout | as permissões efetivas da role de deploy ainda não foram verificadas; é obrigatório coletar policy e receipt de simulação antes de concluir ausência ou recomendar grant. O pipeline também não prova schema hospedado antes da imagem do worker | 2026-09-07 |
| Fronteira agentic | `codex-review.yml` e inventário tipado | Codex em CI está explicitamente modelado como executor privilegiado com `danger-full-access`, filesystem, comandos, credencial e egress; os fluxos para o store de source/review/log/artifact e para OpenAI incluem risco de `credential_secret`; isolamento de segredo, allowlists, prompt injection, logs e artefatos permanecem gap crítico | 2026-09-07 |
| Limite | baseline do repositório | configuração live, contratos, owners nominais, retenção, regiões, restore, relação ambiente/dado, logging, boundary agentic e supply chain permanecem 18 gaps; nenhuma alegação de SOC 2, ISO, pentest ou compliance | 2026-09-07 |
...
* `supabase/tests/project_review_roles.sql` executed on staging `gjkkjtbfnssdsbmlhmwk` through `execute_sql` inside `begin; ... rollback;` after applying `project_review_roles`: result `project_review_roles_passed`. Cases: open-mode member keeps every action and the approval record is persisted; plain member cannot manage roles; foreign tenant gets not found on assignment, context and approval and reads zero rows; assignments are idempotent and refuse non-members and unknown roles; read-only member, preparer and reviewer are denied approval; the documentary, institutional, provider and work-request commands deny the read-only member and admit the preparer up to their own pre-existing checks; a reviewer's return is recorded and the version is no longer approvable while the preparer's own adjustment is not a review decision; self-approval is refused by default, allowed through the organization setting, refused again by a project `forbidden`, allowed by a project `allowed`; the approver approves exactly once with idempotent replay and the record is persisted on the dispatch, the accepted event and the approval projection; a document version change supersedes the approval and re-approval is refused.
* Staging recorded the migration as version `20260910193505` under the name `project_review_roles`; the applied statements are byte-identical to `supabase/migrations/20260910230211_project_review_roles.sql` (md5 `fd1cc2b93a75942114805df234fa7eb8` of the file without its trailing newline). The local file name is aligned to the production stamp at release.
* Security advisor after the migration: 0 lints.
* Vitest: `packages/work-plan` 160 tests (7 new for the registry); `apps/web` component and lib tests for the entry, the roles surface, the review context, the work-request history and the approval record; message parity kept.
* Playwright: `institutional-setup.spec.ts` extended (documentary request explained as not activated, financial request routed and recorded, roles assigned, self-approval refused then allowed); `documentary-work-products.spec.ts` moved to the common entry (still gated by `OFFROAD_E2E_DOCUMENT_WORK_PRODUCT`). Execution happens in CI; no local Supabase stack on this machine.
* Not proven here: production behavior (migration not applied), format policy and templates (steps 3 and 4).
