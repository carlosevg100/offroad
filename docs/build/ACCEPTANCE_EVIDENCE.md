...
Evidence: `pnpm check` green, 43 of 43 tasks, 3.699 tests across 43 suites, including the 30 recorded runs re-executed by the evals harness, 41 files in `credit-playbook`, 17 in `work-plan`, 9 in `release-governance`, 59 in `document-worker` and 100 in `apps/web`. The capability ledger gains `finance.case01-debt-methods` (live, universal, production, `internal_validation` only) and `execution.case01-specialists` stops saying "nine implemented methods".
