...
| Claims externos governados | `security-assurance-statements.ts`, `ASSURANCE_CLAIM_TRUST_BOUNDARY.md` e testes | claim positivo exige scope fingerprint exato, evidência e bytes resolvidos, hash, assinatura Ed25519, trust root, emissor/claim autorizados, validade e ausência de revogação; decisão reconstruída e milestone sem evidência falham fechados. Sem root/evidência real, a vista permanece não certificada/não atestada | 2026-09-07 |
| Lint de prose de assurance | `security-assurance-language.ts` | defense in depth conservadora: somente a saída exata dos renderers tipados é allowlisted; linguagem livre de alto risco, inclusive negativa, futura ou de polaridade mista, volta para revisão em vez de ser interpretada como NLP | 2026-09-07 |
