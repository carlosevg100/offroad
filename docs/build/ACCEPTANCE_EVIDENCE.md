...
| Validador fail-closed | `security-current-state.ts` e `security-current-state.test.ts` | a avaliação síncrona é explicitamente declaration-only; o gate confiável resolve bytes do Git no commit declarado e bytes/hash de observações locais, recusando path inexistente, hash divergente, observação que alegue confirmação, ID duplicado, owner ausente, evidência vencida, referência/backlink inválido e material semelhante a segredo antes do parse | 2026-09-07 |
...
| Fronteira agentic | `codex-review.yml` e inventário tipado | Codex em CI está explicitamente modelado como executor privilegiado com `danger-full-access`, filesystem, comandos, credencial e egress; isolamento de segredo, allowlists, prompt injection, logs e artefatos permanecem gap crítico | 2026-09-07 |
