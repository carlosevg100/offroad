...
## Cobertura contratual explícita no comparativo: diagnóstico v8 opt-in, 08/09/2026

Candidato interno, sem chamada pelo worker, despacho, loader privado ou promoção de capability. A entrada v8 exige escopo declarado dos instrumentos atuais/propostos, cobertura contratual por instrumento e convenções prefixadas, efetivas anuais, mensais sem ajuste de dia útil e juros pagos na carência. Desembolsos nos dias 29–31 são recusados nesta entrada até existir convenção de fim de mês explícita e suportada; isso evita o rollover silencioso de 31/janeiro para março, preservando v7. Despesas de caixa ausentes impedem o cenário transacional; zero explícito permanece válido. Cobertura desconhecida permite comparação econômica parcial. Nenhum covenant é mensurado nesta versão inicial; métricas diferentes são identificadas separadamente e nunca convertidas automaticamente em alavancagem. Não há headroom agregado, conclusão de conformidade ou ranking por headroom.

A matemática é compartilhada com o executor existente. A entrada v7, seu procedimento e o Case01 congelado permanecem inalterados; regressões preservam fingerprints originais. Datas de conhecimento/revisão são distintas da data econômica e da data da operação proposta. Referências têm versão, hash e localização; localizações distintas do mesmo documento são válidas. O fingerprint da saída inclui cálculos e identidade do input. Testes sintéticos cobrem duas moedas e refinanciamento parcial com principal remanescente, prêmio de saída e despesas explícitas, além de ausência/contradição de cobertura e convenções recusadas.

Validação de referências não autentica revisão, universo contratual, fonte vigente, nominal ou preço de saída. O output declara diagnostic_only/reference_consistency_only. Sobrevivência de obrigações após quitação, completude do ledger, documentos/aditivos e aprovação exata continuam responsabilidade do futuro loader autorizado; nenhum selo de produção é inferido do schema. Sem DB, autorização, telemetria, provider ou fórmula novos. Revisão independente encontrou e resolveu identidade indevidamente sensível ao locator e fingerprint sem vínculo completo. Gate local completo aprovado em Node 24.19.0: lint, typecheck, testes e build, 43/43 por etapa. Inclui 339 testes de credit-playbook (24 novos), 315 web, 299 worker e 89 evals. Revisão independente final sem bloqueador no escopo delimitado. CI e integração privada não demonstrados nesta fatia.
...
Escopo somente leitura do payload autorizado: sem migrações, providers novos, telemetria, mudanças de autorização, fórmulas financeiras ou autorização de divulgação. Os fingerprints demonstram consistência interna, não homologação econômica. Não promove capabilities, materiais institucionais completos, integração privada com worker, matching ou introduções. Gate local `pnpm check` aprovado em Node 24.19.0: lint, typecheck, testes (315 testes web) e build, 43/43 targets em cada etapa. E2E acrescenta navegação por teclado entre achado e fonte, acesso aos métodos e viewport de 390px com inspeção aberta. Quality 34220487935 aprovou a revisão c3e9b92 com 20 E2E e 10 dependentes de provedores pulados; Security passou. Capturas reais confirmaram 390px sem overflow e fontes recolhidas, com navegação por teclado testada. A montagem condicional dos métodos e a integração com main #550 passaram novo gate local completo, 43/43 por etapa; o E2E agora exige ausência do conteúdo dos métodos no DOM antes de abrir e após fechar. PR #549 integrada em main ea6d233a3fabf58179b343f0324da58b846303cc. Quality34221726124 e Security34221726093 passaram; E2E20/10provider skips (2,3min), capturas finais desktop/390px revisadas. Produção Vercel dpl_BssdnfJAxxqcFiMjfzNbCX8zkG5y READY, página pública PT e projeto autenticado existentes verificados em leitura. O projeto histórico conserva fallback explícito (não tem novo DecisionContract); seus nove métodos abriram/fecharam corretamente, com montagem 0→9→0. Worker 34222615546 concluiu, PRIMARY task 280 exata e capacidade positiva verificadas. Main local sincronizada sem alterações. A prova de navegador da nova leitura completa está no E2E, sem reescrever snapshots antigos.
