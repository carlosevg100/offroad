...
O mapa de evidências dos catorze critérios do pronto da etapa 18 encontrou onze com prova direta no CI e três sem. Esta PR acrescenta as três, só com testes, sem migração e sem mudança de produto; os casos SQL terminam em rollback e o script grava só no banco descartável do CI. Espera humana resiste a reinício: `scripts/ci/verify-dependency-recompute.mjs`, no job Database e com o laço real do robô, passa a registrar o perfil do método de capital com teto acima de zero (gatilho de armazenamento desligado só para essa linha do banco descartável); a nova revisão da base planeja a candidata da linhagem como espera de uma pessoa, marco `awaiting_human`, sem job e sem lease; o laço em execução chega a ocioso, um cliente novo com outra conta e outro token de robô (novo dono de lease) também, e a candidata e a espera continuam idênticas; o dono autoriza por `authorize_work_update_v1` e o laço reiniciado produz exatamente uma execução, para o solicitante original, com linhagem. A candidata de orçamento zero que o script produzia sem pessoa não cabe no mesmo banco (o capital tem um único perfil executável e uma única release que pode ser pedida); o agendamento automático de orçamento zero continua provado pelo planejador nos testes SQL. Custo confirmado uma vez: a seção 8b de `work_continuation_commands.sql` autoriza a candidata com custo, deixa o lease da primeira reivindicação expirar e produz a candidata com a segunda; uma execução, o lease velho recusado, nenhuma espera nova, autorização e teto usados uma vez; a execução produzida tem uma conta de orçamento em zero, a reserva do teto autorizado é recusada (`execution_operation_denied`) e a reserva de custo zero da única operação se repete sem somar, também sob o lease de um job reivindicado de novo. Grafo incompleto: a seção 31 de `work_continuity_dependencies.sql` aposenta pelo comando do operador a única release de um procedimento fixado por uma execução; o evento seguinte do procedimento grava o fato `graph_incomplete` com a lacuna `head_unknown`, o planejador retém a execução com o sinal `method_release` do procedimento e nada é planejado nem reaproveitado, embora a fonte fixada não tenha mudado; a release seguinte libera a retenção e o próximo planejamento produz uma candidata. Limite registrado: o incremento fechado da etapa 17 recusa qualquer reserva com custo e exige orçamento zero no contrato, então a variante com uma reserva acima de zero e a sua repetição precisaria de mudança de produto e não foi feita; o teto autorizado fica na candidata. A guarda de proveniência do capital (etapa 17) só aceita o manifesto publicado e a identidade do manifesto exige o mesmo hash na release: as duas primeiras rodadas, com uma segunda release de capital, foram recusadas (`execution_payload_provenance_denied`, depois `execution_manifest_identity_mismatch`), e publicar a próxima versão do capital exige atualizar essa guarda na mesma migração. A aposentadoria de uma release não emite evento (decisão do 3A), então o fato só aparece com o evento seguinte do procedimento; o planejamento continua correto nesse intervalo. Tabela dos catorze critérios, com prova, job do CI e tipo, em `docs/build/arcabouco/etapa-18-6b-provas-do-pronto.md`.
