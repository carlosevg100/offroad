## Etapas 20 e 21 preparadas como documentação, em paralelo com a 19, 26/09/2026

Os planos de execução das etapas 20 (revisão, aprovação e decisão presas à revisão exata) e 21 (exportação governada e reimportação antes do Office nativo) estão preparados como documentação em `docs/build/arcabouco/etapa-20-execucao.md` e `docs/build/arcabouco/etapa-21-execucao.md`, como o roteiro permite na seção 19 e como o OK do fundador de 26/09/2026 registra. Cada plano traz o ponto de partida medido no código de `main` (arquivo e linha, sem consulta a banco), as decisões de desenho, os incrementos com ordem de publicação, os critérios do pronto como casos de teste concretos, o que fica fora, os riscos com dono e a seção "O que depende de decisão anterior", que lista os incrementos da etapa 19 que precisam entrar antes. Nenhuma migração e nenhum código de produto foram criados: a migração `revision_bound_reviews_and_decisions` (20) e a migração `artifact_import_candidates` (21) esperam o OK seguinte do fundador; só os incrementos de contrato e testes, sem migração, estão cobertos pelo OK vigente. Medições registradas nos planos: nenhum dos alvos das duas etapas existe hoje; a aprovação do execution brief é o único ato que já prende versão exata e registra preparador e revisor; a decisão sobre artefato prende o fingerprint mas não o revisor nem a alçada; as decisões propostas pelo robô não têm ato humano; o único caminho de reimportação é a proposta institucional em SQL, sem chamador web em `main`; as planilhas exportadas não têm nomes definidos, o DOCX não carrega fingerprint de material e o PPTX só carrega ids nos nomes de forma das listas.
