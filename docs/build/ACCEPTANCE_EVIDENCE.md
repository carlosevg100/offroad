## Stage 1B resumed: database boundary applied, application rollout pending

The terminal-metadata guard correction and the three stage 1B migrations are applied in production under versions `20260915204111`, `20260915204116`, `20260915204120` and `20260915204124`. Staging holds the same changes under its own journal stamps. The preparatory correction was internal to the approved stage and required no new founder approval. Explicit execution approval and RLS regressions passed after it. Production journal coverage and the reviewed access surface pass for all 312 file versions and 1385 objects; staging has 1446 objects. CI, merged-commit web/worker deployment and the 111-object rotation remain required before completion. The following block report is historical.

## Stage 1B production rollout blocked, 15 September 2026

Production rejected the principal 1B migration with `execution_brief_approval_required` during historical job authorization backfill. Rollback was verified: no 1B resource table and zero 1B journal entries; all 111 private objects remain unmoved. Twelve succeeded historical jobs have no currently valid dispatch. The same metadata-only failure was reproduced in staging. A narrow preparatory guard correction is proposed and passed rollback-only staging evaluation: terminal authorization metadata can change, while unapproved requeue and payload changes remain denied. It is not applied in either environment. Stage 1B is not complete; rollout is paused under the founder's stop-and-report rule. See `docs/build/arcabouco/etapa-1b-rollout-block.md`. Synthetic staging users, organizations, objects, resources and worker token were cleaned and counted at zero.
