## Etapa 19, incremento 2: a capacidade do robô (2a, PR #805) e a migração A do protocolo de revisão (2b), 26/09/2026

Duas PRs sobre `main` `b1d56d62`, depois do contrato do incremento 1 (PR #806). 2a: `artifact-revision.v1` entra em `ANNOUNCED_WORKER_RUNTIME_CAPABILITIES` (`apps/document-worker/src/runtime-schema.ts`), conhecida e não exigida, então a imagem sobe contra o banco de hoje e é implantada antes da migração; o log de boot relata `announcedPresent`; o incremento 4 move a entrada para a lista exigida. 2b: migração **nova** `supabase/migrations/20260927120000_artifact_revision_protocol.sql` (carimbo provisório): `public.artifacts`, `public.artifact_revisions`, `public.artifact_blocks` e `private.artifact_dependency_links`, com RLS pela leitura do trabalho e nenhum DML de cliente; `private.validate_artifact_manifest_v1` com a forma e os nomes de regra do contrato; `private.create_artifact_revision_v1` (núcleo único, replay por fingerprint, pino de direitos resolvido ou recusado, fingerprint do recibo obrigatório, substância só nas escritas novas); `public.create_artifact_revision_v1` (pessoa) e `public.worker_create_artifact_revision_v1` (robô, pela capacidade do job); `public.read_artifact_revision_v1` e `public.read_artifact_head_v1` com `release` (`internal | released | blocked`, pelos fatos de aprovação de hoje sobre a versão exata) e `freshness` (`current | stale | unknown`, pelos fatos e candidatas da etapa 18 e por versões novas de fonte); gatilhos `after insert` de projeção nos quatro depósitos históricos (`work_product` para artefatos de projeto e manifestos de caso, `model_result` para resultados institucionais concluídos, `material` para `material_artifact`), backfill idempotente com origem `legacy` e contagens em `raise notice`; capacidade adicionada ao contrato de runtime por patch de texto (agulha `"dependency-recompute.v1","dependency-recompute-health.v1"]'::jsonb`, exceção `artifact_revision_capability_contract_changed`), snapshot do corpo efetivo atualizado (sha256 `352cbda9`). Provas: **novo** `supabase/tests/artifact_revision_protocol.sql` (quinze blocos com o pronto do plano), extensão de `rls_non_interference.sql` e de `worker_runtime_schema_contract.sql`, testes do robô; rodadas numa réplica local descartável (Postgres 18, 401 migrações do zero, corpos efetivos iguais ao snapshot). Tipos da web: quatro RPCs públicas adicionadas à mão. Inventário da etapa 0 e catálogos não editados: o lead aplica em staging e produção, renomeia, recaptura o corpo do contrato, regenera os tipos e concilia. Nota de desenho: `docs/build/arcabouco/etapa-19-2-protocolo-de-revisao.md`.
