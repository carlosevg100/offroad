...
As migrações 20260922131651, 20260922132215, 20260922132931 e 20260922150256 estão somente em staging nesta candidata. Dez suítes SQL novas e três regressões de política passaram com rollback; zero linhas sintéticas permaneceram. A primeira suíte de fonte expôs hash declarado sem recibo; a segunda migração corrige o caminho e a mesma negativa passou. A prova de expiração dentro da mesma transação também falhou antes da terceira migração; o relógio corrente agora é usado no avaliador único de política e na participação em grupos. A negativa e as regressões passaram depois da correção. A revisão independente encontrou a ausência de revalidação de read atual: reproduzida em fonte e base, corrigida pela quarta migração e negada depois, inclusive em dependência transitiva. A retirada/troca de release depois do claim também foi negada em staging. Revisão dos ajustes, CI completa, catálogo/journals de produção e implantação permanecem gates abertos. Não há completion, produtor, ativação ou avanço para a etapa 18. A correção não reescreve o SQL aplicado nem os métodos publicados.

Riscos da 2B antes de promoção: provar concorrência efetiva, expiração depois de espera por locks e os limites da contabilidade; revisar a ligação de autoridade transitiva. Engenharia de execução responde por esses gates. Consumidor e grants mínimos ficam no incremento 3; proveniência do corpo completo e retenção antes de qualquer egresso continuam obrigatórias nele. A ordem global do legado não foi declarada corrigida por este núcleo. Rollback operacional mantém a superfície nova sem grants e sem registros, preservando os consumidores anteriores.
