...
4A banco: `execution_producer_authority` em staging `20260923211108` e produção `20260923233258`; `execution_producer_commands` em staging `20260923212248` e produção `20260923233409`. Provas `execution_producer_authority.sql` e `execution_producer_request.sql` executadas em staging com rollback. Revisão independente do adaptador de perfil aprovada (registro em `knowledge/reviews`); perfil v4 derivado com fingerprint igual ao lock. Tipos públicos com os quatro RPCs novos. Advisors de segurança zero; journals e inventário conciliados. Registro do perfil, grant do espaço do fundador e liberação da v4 em produção ficam registrados no completion; a tela e o compositor do pacote entram na PR seguinte.
