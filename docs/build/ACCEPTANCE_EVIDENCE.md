...
## feat/case01-methods-production: the ten Case 01 methods up the maturity ladder (10 September 2026)

The ten methods of Case 01 sat at `implemented` with their gold case and their adversarial ids written in the frontmatter and nothing executed on record. They now carry the evidence each rung asks for, and seven of them reached `production` on the founder's instruction of 10 September 2026 (Carlos Eduardo Galves, "nada que deva estar em produção fica fora"): `build-debt-ledger`, `reconcile-financial-statements`, `build-interest-and-indexation-schedule`, `reconcile-covenant-definitions`, `diagnose-maturity-wall`, `compare-refinancing-before-after` and `estimate-exit-cost-by-series`. The three whose contract spends a model call stopped at `ready_for_founder`, because that step has no recorded evidence: `declare-scenarios`, `plan-meeting-brief` and `write-meeting-synthesis`. The manifest test refuses to project an approval for a method with a model call, so the rule is enforced and not only written.

Independent reviews: ten records under `packages/credit-playbook/knowledge/reviews/`, one per method, each produced by going back to that method's own sources (procedure Markdown, executor, frozen Case 01 input, answer key) and recalculating the gold numbers outside the executor before comparing them. The eighteen ledger rows sum by hand to 5.670.186 and 4.988.383, the balance-sheet reconciliation closes at zero, the contractual net debt is 5.670.186 + 14.335 menos (1.430.714 + 235 + 25.095) = 4.228.477 against 4.214.377 by the release definition and 4.214.400 reported, six IPCA series are 743.955 (13,12%) and three foreign-currency loans 1.102.582 (19,45%), the debt bridge closes at 5.670.186 and the cash bridge at 1.430.714, the annualized CDI is 0,13899875 and the 14th's first coupon 1.994,73141654 under the indenture's eight-decimal rounding, the implied EBITDA is 895.863,77118644 behind the reported 4,72x, the maturity cover runs 1,16334479 then 0,25858447 with a carried shortfall of 1.804.457, and the extend-di alternative lands at 5.699.383 / 1.468.342 / 4.245.141 and 4,73859983. Every review is `conditional` with its conditions written, and every method that spends a model call says which step it did not cover.

Recorded runs: thirty records under `packages/credit-playbook/knowledge/reviews/runs/`, 130 cases, zero model calls. One gold case per method whose expectation comes from the answer key of Case 01 and not from the result; twenty adversarial cases, one per declared id, including the scale mutation, the compensating split swap, the definition that contradicts the formula, the lease counted as other onerous debt (net debt 4.228.477 to 4.505.245), the opening that does not reproduce 4,72x, the board approvals that never become a source of payment and the fourteen make-whole routes with no quote; ten permutations per method asserting that input and output fingerprints do not move. Arrays whose order is meaning (the operands of a difference, a covenant's tier ladder) are never permuted and signed content is never touched, both declared in the harness. `packages/evals/src/case01-method-runs.test.ts` re-executes the harness and compares every committed record, so a stale record fails instead of aging quietly.

Defect found and fixed before the review closed: the meeting brief planner canonicalizes every set-valued input before it signs (audience, undefined aspects, objects, headlines, questions, coverage search, previous blocks) and the base's document list was the one it missed, so the same base in another order produced a different fingerprint and the object looked new when nothing had changed. Fixed by sorting the list like the rest, with a regression test over three rotations and the reversed list.

Exposure: the seven promoted methods are now a fourth capability of the ordinary project work entry, `debt_structure_analysis`, beside documentary reading, financial result and provider research. The registry names the exact methods it executes with their versions, the inputs they need, the approval gate and the deliverable types; `packages/credit-playbook` pins that list against the methods that actually reached production. The deterministic classifier routes named debt-structure work to them without any semantic router ("ledger de dívida", "definições de covenant", "parede de vencimentos", "custo de saída" and their English equivalents), while words that name a model stay with the financial result. What gates the capability is data, not a grant: the dispatch asks for private access, processed documents, an accepted debt fact under the extraction's `debt.` field paths and a current plan, and without any of them it blocks and names what is missing. The Case 01 preview keeps its own path untouched for the organizations that hold it, and now declares on every artifact the rung its method actually reached instead of a fixed `implemented` label.

Migration `20260911010000_project_debt_structure_capability.sql` (additive; applied to the staging branch under the name `project_debt_structure_capability`; not applied to production by this task). It widens the `capability` CHECK of `public.capital_project_work_requests` and the same list inside `private.record_capital_project_work_request_v1` by one value. No policy, grant or validation was removed, and the generated types do not change because the column is `text`. Rollback: revert the file and re-apply the three-value CHECK and the previous function body; no table, column or row is created or dropped. Supabase security advisors report 0 findings.

Evidence: `pnpm check` green after rebasing on the merged `feat/receivables-method-production`, 43 of 43 tasks, 3.805 tests across 43 suites, including the 30 recorded runs re-executed by the evals harness, 41 files in `credit-playbook`, 17 in `work-plan`, 18 in `release-governance`, 59 in `document-worker` and 100 in `apps/web`. The capability ledger gains `finance.case01-debt-methods` (live, universal, production, `internal_validation` only), keeps `finance.receivables-released-analysis` as the only scope carrying customer work, and `execution.case01-specialists` stops saying "nine implemented methods".

Limits recorded on purpose: the recorded runs execute the frozen Case 01 evidence, and no path builds these executors' inputs from an arbitrary project's documents, so the entry blocks on accepted debt facts and nothing runs on live data yet; the capability is selectable, explained and recorded, and its result surface is not built; the three methods that spend a model call are not covered by the founder's approval; nothing in this work produces a customer deliverable, external material or external action.
