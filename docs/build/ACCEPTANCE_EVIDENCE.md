...
Evidence: `pnpm check` green after rebasing on the merged `feat/receivables-method-production`, 43 of 43 tasks, 3.738 tests across 43 suites, including the 30 recorded runs re-executed by the evals harness, 41 files in `credit-playbook`, 17 in `work-plan`, 18 in `release-governance`, 59 in `document-worker` and 100 in `apps/web`. The capability ledger gains `finance.case01-debt-methods` (live, universal, production, `internal_validation` only), keeps `finance.receivables-released-analysis` as the only scope carrying customer work, and `execution.case01-specialists` stops saying "nine implemented methods".
