## Stage 1C: role-free runtime and entry flow, compatibility phase

The runtime discards legacy professional context before constructing any reasoning request. Main advisor, capital planning, debt view, origination, intent classification, preview questions and the deterministic execution gate no longer consume the person's professional profile. Objective, evidence and method determine analytical scope and rigor; institution capabilities describe execution means only. The professional onboarding form and settings writer are retired; historical settings URLs preserve the authorized workspace while redirecting to work. This is the application-first transition approved in stage 1C; loader migration follows after this worker is deployed. Stage 1C remains incomplete until its migration, SQL proof, main CI and both deployments are recorded.

Synthetic staging reproduction captured all five agent-loader versions for CFO, analyst, advisor and absent profile with identical grants, question and source. Pre-call model requests confirmed profile leakage. New counterfactual tests cover the complete advisor request, classifier, execution gate, objective plan and specialized executors. The updated object inventory records retired entrypoints without changing database receipts. No production migration is part of this compatibility release.
