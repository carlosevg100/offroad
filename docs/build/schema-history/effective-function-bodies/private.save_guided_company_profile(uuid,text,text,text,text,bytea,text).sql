CREATE OR REPLACE FUNCTION private.save_guided_company_profile(p_session_id uuid, p_name text, p_legal_name text, p_website text, p_description text, p_identifier_hash bytea, p_identifier_last4 text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
 perform private.require_resource_access_v1(p_session_id,'work');
  ...
  if not private.can_manage_organization(session_row.organization_id) then
