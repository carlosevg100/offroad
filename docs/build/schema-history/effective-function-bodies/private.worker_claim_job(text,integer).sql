CREATE OR REPLACE FUNCTION private.worker_claim_job(p_worker_token text, p_lease_seconds integer DEFAULT 600)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  where kind not in ('work_execution','governed_evaluation') and ((status = 'queued' and available_at <= now())
    ...
    and kind <> 'execution_brief_proposal' and (not private.requires_execution_brief_approval(id) or private.execution_dispatch_is_current(id,true))
 ...
 if job_row.attempts + 1 > job_row.max_attempts then
  ...
  set status = 'leased',
      ...
      leased_by = worker_id, leased_account_user_id = auth.uid(),
