CREATE OR REPLACE FUNCTION private.enqueue_primary_case_analysis(p_organization_id uuid, p_processing_run_id uuid, p_intake_session_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
declare
  ...
  session_locale text;
  v_controlled_execution_id uuid;
  case_job_id uuid;
...
begin
  ...
  select execution.id into v_controlled_execution_id
  ...
  if v_controlled_execution_id is null then
    insert into public.controlled_case_executions (
      ...
      run_row.created_by
    ) returning id into v_controlled_execution_id;
  end if;
  ...
  insert into public.processing_jobs (
    ...
    jsonb_build_object(
      'locale', session_locale,
      'execution_id', v_controlled_execution_id,
      'execution_mode', 'primary',
  ...
  if case_job_id is null then
    ...
    where job.organization_id = p_organization_id
      and job.controlled_execution_id = v_controlled_execution_id
